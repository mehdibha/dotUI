...
flowchart TD
  ...
  C -->|kind: export, engine| SHADCN["/r/init?doc=<br/>/r/&lt;item&gt;?doc=&format=shadcn"]
  ...
  R -->|resolved Contract| CONTRACT["/r/&lt;item&gt;?doc=&format=contract"]
...
Returns the immutable [Registry Manifest](03-registry.md) snapshot: dotUI's pinned vocabulary at that version. This is the *self-describing* root of the whole system — every axis declaration, every sync group's Style Contract, the baseline [Dimensional Token Graph](05-tokens.md) (~76 semantic tokens, 2 dimensions `scheme` and `contrast`), the icon map, the codeStyle option declarations, and the component roster.
...
```jsonc
{
  ...
  "styleLayers": { /* lifted Style Contracts, engine-neutral */ },
  "syncGroups":  { /* members, syncedAxes, shared contract */ },
...
Returns the `registry:style` base item the shadcn CLI consumes for `npx shadcn init`. It is `compile(resolved, { kind: 'export', engine, codeStyle })` packaged as the base item — the one place the token layer, plugins, and shared lib land.
...
```jsonc
{
  ...
  "cssVars": { /* @theme inline block, if engine=tailwind */ },
  "files": [
    { "type": "registry:lib", "path": "lib/utils.ts", "target": "src/lib/utils.ts",
      "content": "export { cn } from \"cnfast\";\n" }
    /* when engine=stylex: tokens.stylex.ts (defineVars over the same vocabulary)
       replaces the @theme css — the token layer emits for whichever engine the doc pins */
  ],
  "config": {
    "style": "default",
...
### 3.3 `/r/<item>?doc=<ref>&format=<shadcn|contract>` — one resolved item

```
GET /r/button?doc=ds_01J8ZGEIST00@1.0.0&format=shadcn
```

`format=shadcn` (the default) returns the shipped registry item — the component source with its resolved styles, ready to install:
...
```jsonc
{
  ...
  "files": [
    { "path": "ui/button/base.tsx", "type": "registry:ui", "target": "ui/button.tsx",
      "content": "/* tv() config with resolved classes, or stylex.create — per engine */" }
  ]
}
```

The `content` is what the compiler's [EngineEmitter](04-styles.md) produced: for `engine: 'tailwind'`, an idiomatic `tv()` config byte-comparable to a hand-authored file (flatten-on-export means a contract node still pointing at its default emits `bg-primary`, not the var form); for `engine: 'stylex'`, a `stylex.create` module. `codeStyle` AST transforms have already shaped quotes, arrow-vs-declaration, one-line-per-variant, and comment density. The class strings are character-for-character what the preview DOM showed — same `compile`, same output.

`format=contract` serves the engine-neutral [Style Contract](04-styles.md) JSON for the item — slots × dimensions × states × token-typed declarations, fully resolved for this dsdoc:

```
GET /r/button?doc=…&format=contract
→ StyleContract JSON (ir: 1)
```

This is the surface an agent or an alternate emitter reads to understand a component's structure without committing to an engine. It is the *neutral* form; `format=shadcn` is the *engine-emitted* form. Both come from the same resolved Contract; `format` picks the projection.

**Response headers** on `/r/<item>` and `/r/init`: `Cache-Control: public, max-age=60, s-maxage=31536000, immutable` when `doc=` is a pinned `@version` or an inline canonical blob (both are immutable inputs → immutable output), degrading to `s-maxage=3600, stale-while-revalidate=86400` when `doc=` names a *head* (mutable) system. The cache key is `(item, canonical-hash(resolved-inputs), format, engine)` — see §9.
...
A bundle request is `compile(resolved, { kind: 'export', engine, codeStyle })` followed by per-target packaging. The compiler already knows how to emit component sources and the token layer; the profile tells the bundle emitter *how to package* them:

- `themeDelivery: "real-files"` → the token CSS (or `tokens.stylex.ts`) is written as a project file, not a registry `css` field.
...
```
GET /api/export/zip?doc=ds_…@1.0.0&engine=tailwind
→ 200 application/zip
  Content-Disposition: attachment; filename="geist.zip"
```

The whole resolved system as a downloadable archive: the base (theme CSS or `tokens.stylex.ts`, plugins, lib) plus every registered component's shipped source, laid out as an installable folder tree with a `README.md` and a `dotui.lock.json` seeded for later `dotui update`. It is `compile(resolved, { kind: 'export', engine, codeStyle })` packaged per the `zip` profile and streamed through an archiver. No account, no CLI, no third party — the "give me the files" path. The zip profile is just another `TargetProfile` (`framework: null`, `dependencyResolution: registry`, `themeDelivery: real-files`, `layout` = a plain `src/` tree).
...
| `dotui_export` | `(doc, target, engine) → files[]` | the matching `/r/*` or `/api/*` route |
...
```
cacheKey = sha256( route, canonical-hash(resolvedInputs), format, engine, targetProfileId? )
```
...
The lock records *exactly* what was installed and *how it was shaped*: the doc, the manifest pin, the engine, the `codeStyle`, and a **pristine content hash per file**. The pristine hash is the pivot of the whole update story — it lets `plan` tell an untouched file from a hand-edited one without guessing.
...
|---|---|---|---|---|
| **shadcn CLI (Tailwind)** | `/r/init` + `/r/<item>?format=shadcn` | base (theme CSS, plugins, lib) + per-item `tv()` source | byte-comparable to hand-authored; preview = export by construction | `dotui update` — reconcile + `plan()`, clean overwrite / 3-way merge |
| **shadcn CLI (StyleX)** | same, `engine=stylex` | base (`tokens.stylex.ts`) + `stylex.create` sources | computed-style parity with Tailwind (CI-proven) | same `dotui update` |
...
| **Style Contract** | `/r/<item>?format=contract` | engine-neutral Style Contract JSON | exact resolved structure, no engine commitment | re-fetch with new `doc=` |
