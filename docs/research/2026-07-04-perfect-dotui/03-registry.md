...
This chapter specifies the anatomy of a registry item file by file, the seven item kinds and what each ships, the sync-group and axis declarations items carry, the icon abstraction that actually swaps imports at compile time, how a manifest snapshot is built from registry source, and the numbered authoring rules that keep the source clean, portable, and engine-neutral. The worked fixture throughout is **Button** — shared with [Styles](04-styles.md), [Tokens](05-tokens.md), and [Axes](06-axes.md) — with **Menu** (declared vars) and **Loader** (file-variant) as supporting cases.

The registry does not know about Tailwind versus StyleX, about `/create`, about the router, or about DTCG. It authors component behavior and Tailwind-string styles; the [Style](04-styles.md) and [Compiler](11-compiler.md) packages lift, resolve, and emit. The one hard boundary — the registry imports only engine-neutral surfaces and published packages — is what lets the same source feed two engines and six export targets. This chapter's normative rules exist to hold that boundary.
...
> **Why not keep styles authored outside the registry** (as `@/lib/styles` is today)? Because the styling engine is *part of the item's authored form*. When `defineComponentStyles` lives in `@dotui/style` and the item imports it as a published package, the item is self-contained: it can be lint-checked, lifted to a Style Contract, and reasoned about without reaching into an app. The registry authors styles; it does not author the style *framework*.

**Tradeoffs (§1).** One package for ~72 components plus blocks is large, and a change to a shared helper in `@dotui/style` fans out to every item's lift. We accept this: the alternative — per-component packages — multiplies publish overhead and version skew for no authoring benefit, since items already share one manifest version. The registry is versioned as a whole; the manifest snapshot is its release unit.
...
|---|---|---|
| `base.tsx` | yes (transformed) | The behavior template — RAC primitive + explicit slots + the style seam |
| `styles.ts` | no (lifted) | Full parametric style definition (`defineComponentStyles`) |
...
The split is the same discipline as today's registry, sharpened: one canonical authored form per concern, and a clean line between *authored source* (`base.tsx`, `styles.ts`) and *derived artifacts* (the Style Contract, the manifest slice, the shipped file). Nothing is hand-maintained in two places.

### 2.1 `base.tsx` — the behavior template

`base.tsx` is what the user receives via the shadcn CLI, after the compiler resolves its `./styles` import into a static per-engine style call. It is plain React Aria Components plus one style seam. The single authored file feeds both engines: the [Compiler](11-compiler.md) emits a Tailwind `tv()` call or a StyleX `stylex.create` call into the same template, above the neutral component line.

Two rules shape the template beyond ordinary RAC usage. First, **relation slots are explicit props, never children scanning.** A button that changes its padding when it has a trailing icon reads that from a `suffix` prop, not by walking `children` for an `<svg>`. This is the load-bearing decision from the [Styles](04-styles.md) chapter: `hasIconEnd` becomes a deterministic boolean the Tailwind `:has()` binding and the StyleX runtime boolean both read from the same truth source (rule R6, §9).

Second, **every rendered slot carries a `data-slot` attribute.** The normalization pass that enforces the owned-slot invariant (§4 of [Styles](04-styles.md)) relies on slots being addressable; `data-slot="spinner"`, `data-slot="content"`, `data-slot="icon"` are the anchors both the emitted CSS and the runtime target.

Here is Button's template in the canonical authored form. It reads almost exactly like today's file — the difference is the explicit `prefix`/`suffix` slot props and the neutral style seam:
...
interface ButtonProps
  extends React.ComponentProps<typeof ButtonPrimitive.Button>,
    ButtonVariants {
  isIconOnly?: boolean
  prefix?: React.ReactNode // leading icon slot — drives the `iconStart` relation state
  suffix?: React.ReactNode // trailing icon slot — drives the `iconEnd` relation state
}
...
The template is neutral above the `styles(...)` line — the same behavior, the same slots, the same relation attributes for both engines. Only that one line differs in the emitted output: `className={styles(...)}` for Tailwind, `{...stylex.props(...)}` for StyleX. Wrapper-level code-style options (arrow vs declaration, file layout) apply identically because everything above the seam is engine-agnostic.

### 2.2 `styles.ts` — the parametric style definition

`styles.ts` is the full style definition, with every parameter. It is authored as Tailwind utility strings in a `tv()`-shaped config — the format contributors already know, and the format that lets them copy-compare against shadcn (rule R8, §9). It is never shipped as-is: the compiler *lifts* it into a **Style Contract** (the engine-neutral IR from [Styles](04-styles.md)), and the emitters produce the shipped Tailwind or StyleX from that Contract.
...
- **`base` is a `tv()` config of Tailwind strings** under a closed whitelist. Every utility lowers to a `(PropKey, TokenValue)` pair; every variant lowers to exactly one state or structural pattern. The `dotui/style-subset` lint *is* the compiler's lowering pass in dry-run mode, so lints-clean ⇒ compiles (rule R7, §9). A hardcoded design value with an available token (`bg-[#635bff]`, `rounded-[7px]`) is a lint error carrying the token hint — CLAUDE.md's hardcoded-value rule enforced structurally.

- **`sizes()` is the canonical way to author density × size geometry.** The raw button `styles.ts` today hand-authors three near-identical density blocks — 150+ lines of duplicated `h-`/`px-`/`gap-`/`size-` ladders across the registry. `sizes()` renders the density × size table once; the compiler folds it into `dims:{density:X, size:Y}` pins on ordinary rules. Adding a fourth density tier is a data edit in one table, not a rewrite of 48 files. This is not optional sugar — new components must use it, and ad-hoc per-density ladders do not pass review (rule R9, §9).

- **`[--color-disabled:var(--neutral-500)]` is a declared var, not a lost one.** The `primary` variant writes CSS custom properties. The lift extracts these into `declaredVars` on the Contract, scoped to the `variant=primary` value, so *both* engines emit them. This structurally fixes the class of bug where a variant's var writes previewed correctly but were stripped on export.
...
`highlight` is an **axis** (§5) whose two values (`subtle`, `accent`) each contribute a Style Contract delta — class slices, CSS vars, or both. The `vars` are captured as `declaredVars` on the Contract's `highlight` dimension so the accent-highlight look survives export in both engines. Each named-style value resolves to a *complete* Contract before anything downstream reads it: the delta is an authoring convenience the compiler erases, so forking, diffing, and LLM generation all operate on whole documents.
...
- **Members share one Style Contract** (`syncGroup` on the Contract, §2.2 of [Styles](04-styles.md)). Authoring a shared fragment — the fill treatment, the hover effect — is one edit read by every member.
...
flowchart LR
  ...
  CON["shared Style Contract<br/>+ button-like token contract"]
  SG --> BTN
  SG --> TGL
  BTN --> CON
  TGL --> CON
  CON -->|one edit restyles both| OUT["resolved Contracts<br/>(both members)"]
```

**Tradeoffs (§3).** Sharing one Contract across members means a member with a genuinely divergent structure (ToggleButton's `selected` state) needs that state modeled in the shared Contract, not bolted on. The detach hatch covers per-axis exceptions; a member that must diverge *structurally* is a signal it does not belong in the group. We accept a slightly higher bar for group membership in exchange for zero accidental drift.
...
|---|---|---|---|
| **UI** | `registry:ui` | transformed `base.tsx` | The main kind; RAC + lifted styles, §2 |
...
Virtual dependencies (`utils`, `focus-styles`, the theme) are bundled into the init payload rather than served as separate addressable items, so `dotui init` writes one coherent base and per-component deps are relieved of them. When the engine is StyleX, the base item ships `tokens.stylex.ts` (`stylex.defineVars` over the same semantic vocabulary) in place of the `@theme` CSS — the token layer emits for whichever engine the dsdoc pins.
...
| `styleLayers` | lifted `styles.ts` → Style Contracts | Per-component base/density/axis layers, engine-neutral |
...
The build is deterministic. It globs the registry, lifts each `styles.ts` to its Style Contract (running the `dotui/style-subset` lowering — a component that does not lint does not build), folds contract and axis declarations in, snapshots the baseline token graph, and content-addresses the whole thing. The manifest is committed; CI's manifest-drift job diffs it against a fresh build, so the pinned snapshot and the source can never silently disagree.

```mermaid
flowchart TD
  SRC["registry source<br/>ui/*/{base,styles,contract,axes,meta,types}<br/>sync-groups.ts · icons/icon-map.ts · baseline token graph"]
  LIFT["lift styles.ts → Style Contract<br/>(style-subset lowering; fails on unmapped class)"]
...
**`styles.ts`** authors the full look: the base classes, six `variant` values, the `isIconOnly` boolean, and the `sizes()` density × size table (§2.2). The `primary` variant's `[--color-disabled:…]` writes become `declaredVars`.
...
**`base.tsx`** is the RAC template with explicit `prefix`/`suffix` slot props, `data-slot` attributes, and the neutral style seam (§2.1).
...
The compiler lifts `styles.ts` into a Style Contract slice — the same one shown in the [Styles](04-styles.md) chapter, where `has-data-icon-end:pr-2` becomes a root-owned `paddingEnd` decl gated on the `iconEnd` relation state, `**:[svg]:…size-3.5` is re-homed to the `icon` slot, and `[--color-disabled:…]` becomes a `declaredVar`. From that Contract:

- The **Tailwind emitter** produces a `tv()` call byte-comparable to a hand-authored file, folded into `base.tsx` above the seam.
- The **StyleX emitter** produces `stylex.create` with boolean-composed states, folded into the same template.
...
One authored folder; two engines; one manifest slice; zero hand-maintained duplicates.
...
- emit Tailwind or StyleX — the [Style](04-styles.md) emitters do, from the Contract;
...
These rules are enforced by registry lints (rule R11 of the [Testing](13-testing.md) canonical list) — style-subset, import boundaries, id permanence, contract integrity — not by convention. A violation fails the build.
...
2. **No hardcoded design values.** The test is *would two design systems disagree on it?* If yes, it goes through a token or contract node (`bg-primary`, `rounded-(--btn-radius)`), never a literal (`bg-[#635bff]`, `rounded-[7px]`). Color, radius, and density-affected property families accept only token/semantic/mix/componentVar values — a literal there is a typed lint error carrying the token hint. Component mechanics (hairlines, hit areas, internal layout, `top-1/2`) stay plain values and are allowlisted.

3. **Flag the missing axis.** A look with no covering axis is not solved by inventing a token. It is flagged as a missing axis — a typed compile error naming the property family — so the gap surfaces as a product decision, not a silent hardcode.
...
6. **Explicit slot props for relations.** Relation states (`iconStart`/`iconEnd`, addons) come from explicit slot props (`prefix`/`suffix`), never children scanning. Every rendered slot carries a `data-slot` attribute so both engine bindings read the same truth source.

7. **Style subset.** Every class in `styles.ts` lowers to a `(PropKey, TokenValue)` pair (utilities) or exactly one state/structural pattern (variants), under the closed `dotui/style-subset` whitelist. The lint *is* the compiler's lowering in dry-run mode: lints clean ⇒ compiles. Out-of-subset classes (class-name-referencing selectors, un-tokenized hexes) are lint errors with mechanical fixes.
...
The registry is one package of authored items, each a folder with one home per concern: `base.tsx` (RAC behavior, explicit slot props, the neutral style seam), `styles.ts` (`defineComponentStyles` + `sizes()`), `contract.ts` (`defineContract`/`surface`/`scalar`), `axes.ts`/`meta.ts` (axis and sync-group declarations), `types.ts` (API docs), and demos. Seven item kinds cover UI, hooks, lib, icon sets, the style base, blocks, and the served manifest. Sync groups make Button ⇄ ToggleButton share one Contract and one set of synced axes with a declared detach hatch. The icon abstraction is a real map the compiler uses to rewrite imports to the user's chosen library. `pnpm build:manifest` folds every axis, contract, style layer, sync group, and the baseline token graph into an immutable, content-addressed snapshot the compiler pins against. Ten normative authoring rules — enforced by lints, not convention — keep the source portable, engine-neutral, and free of hardcoded design values. The registry authors; everything downstream is a pure function of it.
