...
type TokenValue =
  ...
  | { kind: 'mix';   space: 'oklab'|'oklch'|'srgb'; stops: [TokenValue, number, TokenValue] }
...
interface BaseNode {
  ...
  values: Record<CellKey, TokenValue>
}
...
The one relaxation, ratified by the constitution, resolves a cross-panel conflict: a **component decl may reference a component-contract node OR a semantic node** — never a primitive. The tokens judge's stricter "only layer-3" prose is overruled; a slice like `bg-primary` in a variant string references the semantic `color-primary` directly, and the [Style Contract](04-styles.md) records that as a token reference the emitter later flattens or preserves (`codeStyle.tokenIndirection`).
...
- **The single source of truth concentrates risk.** A bug in `resolve()` hits Tailwind, StyleX, DTCG, preview, and verification at once. The parity tests that today catch drift between three emitters become invariants on one heavily-exercised path (constitution §10; [testing chapter](13-testing.md)).
...
- **StyleX consumers can't re-run producers in-browser.** The oklch/contrast math runs in JS at compile time; a StyleX consumer who changes a seed must re-export through dotUI. "Equal fidelity" holds for *output*, not for live in-consumer regeneration (the CSS-var preview does regenerate live — §9).

---

## 6. Resolution and emission: one resolver, three backends

`resolve(graph)` is one topological sweep producing an engine-neutral `ResolvedGraph` in which **references stay symbolic**. Tailwind wants `var(--color-bg)`, StyleX wants `vars.colorBg`; both are "reference to node X," serialized differently. No backend re-derives anything from CSS text.
...
### 6.1 Tailwind v4 backend — exact emitted CSS
...
This one emitter subsumes today's three separate paths (`emitPrimitivesCss`, `emitCss`, per-component `styles.css`). The parity tests between them become a single generation, guarded by two invariants ([testing chapter](13-testing.md)): a property test that **browser cascade ≡ compiler resolution** for random graphs and cubes, and a **Tailwind↔StyleX resolved-value equivalence** test.

### 6.2 StyleX backend — `defineVars` + `createTheme`

`defineVars` over the base cell gives the canonical handles. **Media-bound options emit as conditional values inside `defineVars`** (StyleX supports this natively), so `prefers-color-scheme` / `prefers-contrast` modes work with no JS shim. **Class/data-bound options** (brand, a manually-forced dark) emit as `createTheme` per option, containing only that option's delta. Explicit compound cells become an additional **compound theme documented to apply last** (the arbiter's ruling on ordering):

```ts
export const vars = stylex.defineVars({
  neutral50:  { default: 'oklch(0.985 0 0)',
                '@media (prefers-color-scheme: dark)': 'oklch(0.145 0 0)' },
  neutral950: { default: 'oklch(0.13 0 0)',
                '@media (prefers-color-scheme: dark)': 'oklch(0.985 0 0)' },
  colorPrimary: {
    default: vars.neutral950,               // symbolic ref preserved
    '@media (prefers-color-scheme: dark)': vars.neutral50,
  },
  colorBorder: {
    default: vars.neutral300,
    '@media (prefers-contrast: more)': vars.neutral500,
  },
  colorFgMuted: {
    default: vars.neutral600,
    '@media (prefers-color-scheme: dark)': vars.neutral400,
  },
  btnBgPrimary: vars.colorPrimary,
  btnFgPrimary: { default: vars.onNeutral950,
                  '@media (prefers-color-scheme: dark)': vars.onNeutral50 },
  btnRadius: 'calc(0.375rem * 1)',
  btnFontWeight: '500',
})

// class/data-bound options — manual forcing:
export const schemeDark = stylex.createTheme(vars, { /* dark delta only */ })
export const contrastHc = stylex.createTheme(vars, {
  colorBorder: vars.neutral500,
})
// compound cell — documented to apply last:
export const schemeDarkContrastHc = stylex.createTheme(vars, { /* the compound-key delta */ })
```

The StyleX button consumes `vars.btnBgPrimary`; the Tailwind button consumes `bg-(--btn-bg-primary)` — **the same node id, serialized twice**. Equal fidelity is structural because the engine was never in the token model, only in the final serializer.

### 6.3 DTCG / Figma backend
...
`applyEdit` runs `findCycle` (none), `findLayerViolation` (a semantic referencing a primitive — legal), `findDuplicateName` (unique) → committed. The node now emits in every backend and every cell and appears in every picker whose pool admits `accent`.
...
- the **Tailwind** export gains a `[data-scheme="dim"]` delta block and a `--color-brand-glow` declaration;
- the **StyleX** export gains a `schemeDim = stylex.createTheme(...)` and a `colorBrandGlow` var (a `color-mix` value);
