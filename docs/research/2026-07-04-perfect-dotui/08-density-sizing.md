...
Density is the axis that scales a whole design system's geometry — heights, paddings, gaps, icon sizes, corner radii, type — up or down as one system-wide decision. It is what separates a Material-3-style comfortable system from a Linear-style compact one, and it is one of the hardest axes to model correctly, because "make everything smaller" is *not* a linear operation. This chapter specifies how the perfect dotUI models density: as a **Contract dimension** (`role: 'density'`) whose cells are hand-authored geometry tables written through the `sizes()` helper, resolved by the compiler, and emitted into both the Tailwind and StyleX engines.

The one-sentence version: **density is three named geometry tables per component, folded into the [Style Contract](04-styles.md) as a dimension, resolved by `resolve()`, and either baked into classes or shipped as a runtime `data-density` axis at export — never a CSS multiplier, never a continuous slider.**
...
## 2. Why density is a Contract dimension

The [Style Contract](04-styles.md) (see chapter 04 for the full schema) models every component as `slots × dimensions × states × token-typed declarations`. A `Dimension` has a `role`, and density is one of the four roles:
...
interface Dimension {
  ...
  values: string[]
  default: string
  role: 'style' | 'size' | 'boolean' | 'density'
}
```

Density is a **peer dimension** sitting alongside `variant` (role `style`), `size` (role `size`), and booleans like `isIconOnly`. A concrete geometry rule pins some subset of the dimensions:

```json
{ "slot": "root",
  "when": { "dims": { "size": "md", "density": "default" } },
  "decls": [{ "prop": "height", "value": { "token": "spacing.8" } }] }
```

The rule that button's `md` size at `default` density is `h-8` lives in exactly one place — a rule pinned on `{size:'md', density:'default'}`. Compact's `md` is a *different* rule pinned on `{size:'md', density:'compact'}` declaring `h-7`. The compiler resolves the active pins; the emitter renders them. Density is not special-cased anywhere in the pipeline — it flows through `resolve()` and both `EngineEmitter`s exactly like `variant` and `size`.

This has three consequences that the two rejected alternatives (§3) cannot match:

1. **A density change is a variant-prop flip.** Selecting a tier changes which pin resolves. No `tv({extend})` re-chaining, no recomposition cascade — the [structural tier](10-builder.md) refolds the affected components' Contracts and hot-swaps their class maps in a few milliseconds (§9). It is exactly as cheap as switching a default variant.
2. **Both engines get density for free.** Because density is a dimension of the neutral Contract, the Tailwind emitter renders it as variant classes and the StyleX emitter renders it as boolean-keyed styles — from one authored table. There is no second density mechanism per engine.
3. **A fourth tier is a data edit.** Adding a tier adds a value to the dimension and a column to every component's geometry table. The *machinery* does not change (§10).

### Tradeoffs (density as a dimension)

- **Cross-product cost.** Density multiplies the resolved rule count: a component with 4 sizes × 3 densities has 12 geometry cells to author and resolve, versus 4 without density. The `sizes()` authoring model (§4) collapses the *authoring* cost to one table, and the [structural-tier](10-builder.md) refold keeps the *runtime* cost to milliseconds, but the Contract JSON itself is larger. This is the honest price of hand-tuned geometry, and the design pays it deliberately.
- **Every component must author all three tiers.** The Contract requires a `default` cell; `compact` and `comfortable` are strongly encouraged and, for any component whose geometry differs by tier (nearly all of them), required by review. A component that ships only `default` renders identically at all three tiers — sometimes correct (a component with no intrinsic geometry) but usually a gap. The registry lint flags a component whose `compact`/`comfortable` tables are byte-identical to `default` as a likely authoring omission, not an error (some components genuinely don't scale).
...
Density folds into the Contract as pins, but nobody hand-writes twelve `{when:{dims:{size,density}}}` rule objects per component. The canonical authoring surface is **`sizes()`** — the helper that renders a `density × size` geometry table into the density layer of `styles.ts` before the Contract lift runs. Per the [decision log](00-decision-log.md) and the [style-IR ruling](04-styles.md), `sizes()` is **not optional sugar**: new components author their density ladders through it, and ad-hoc per-density tv layers do not pass review.
...
interface GeometryRow {
  ...
  icon?: TokenScalar       // default icon box → the icon slot's size (re-homed, §6)
...
Every field is optional; a row declares only the properties that differ at that cell. Fields resolve against the [Dimensional Token Graph](05-tokens.md) — `h: 8` is `spacing.8`, `text: 'sm'` is `fontSize.sm`, `radius: 'sm'` is the radius ramp — so a density row never contains a raw pixel value that isn't a deliberate literal (`text: '0.625rem'` is permitted for the hand-picked case; `h: '30px'` is a lint error carrying the token hint, per the hardcoded-value rule).

### 4.2 One table replaces triple-authored ladders

Without `sizes()`, a component authors three parallel tv layers — `compact`, `default`, `comfortable` — each repeating the full slot/variant shape, each spelling out class strings by hand. Across ~72 components this is the source of the largest duplication in the registry: the same size ladder, written three times, hand-kept-consistent by review. `sizes()` collapses it. One call, one table, keyed by tier then size, and the helper renders it into the density-dimension pins the lift consumes.
...
### 4.4 What the lift does with the table

The `density` layer of `styles.ts` is a `sizes()` call. At `pnpm build:registry`, the [style lift](04-styles.md) evaluates it (it is a pure helper — evaluated and inlined like any fragment) and folds each cell into density-dimension pins on the Contract. From the table above, `default md` becomes rules such as:

```json
[
  { "slot": "root", "when": { "dims": { "size": "md", "density": "default" } },
    "decls": [
      { "prop": "height",  "value": { "token": "spacing.8" } },
      { "prop": "paddingX","value": { "token": "spacing.2.5" } },
      { "prop": "gap",     "value": { "token": "spacing.1.5" } },
      { "prop": "fontSize","value": { "token": "fontSize.sm" } }
    ] },
  { "slot": "icon", "when": { "dims": { "size": "md", "density": "default" } },
    "decls": [ { "prop": "size", "value": { "token": "spacing.3.5" } } ] },
  { "slot": "root", "when": { "dims": { "size": "md", "density": "default" }, "states": ["iconEnd"] },
    "decls": [ { "prop": "paddingEnd", "value": { "token": "spacing.2" } } ] }
]
```

Note three normalizations from chapter 04 at work: the `icon` measurement re-homes to the **icon slot** (§6, owned-slot invariant); `iconPad` becomes a **root-owned** `paddingEnd` gated on the `iconEnd` relation state; and everything else stays a plain root decl pinned on `{size, density}`. Density authored as one row, resolved into the same neutral rule shape as every other geometry.

### Tradeoffs (`sizes()`)

- **A closed geometry vocabulary.** `GeometryRow` covers the fields real controls need (h, padding, gap, text, icon, radius, derived vars). A component that needs a geometry property outside that set — a bespoke `clip-path` inset that scales with density, say — either extends the row type (a small, reviewed data change) or drops to a raw density-dimension pin with an explicit note. The fixtures need no such escape; the vocabulary is sized to the registry's real needs, and widening it is a data edit, not a rewrite.
...
The lift turns each `vars` set into `declaredVars` on the Contract — CSS custom properties written at that cell — and the slots reference them via `{calc}` declarations. The [Tailwind emitter](04-styles.md) renders this as the real authored form (`[--input-h:--spacing(8)]` on the cell plus `h-[calc(var(--input-h)-var(--addon-button-inset)*2)]` on the addon); the StyleX emitter renders local vars plus a `calc()` string. Same token graph, identical geometry across engines — the [parity CI](13-testing.md) proves the computed heights match.

The density mechanics here are notable: the *entire density ladder* for input is these eight numbers per cell. Shifting input from default to compact is shifting `--input-h` from `spacing.8` to `spacing.7` (and its siblings) — and every slot that reads the derived set follows. This is the derived-token pattern doing density's work: one authored set per cell, many computed measurements, all engine-neutral.

---

## 6. Density and the owned-slot invariant

Density interacts with two structural rules from chapter 04, and both matter for geometry.

**Icon sizing re-homes to the icon slot.** The authored `icon` field in a `GeometryRow` is conceptually "how big are icons in this button." In Tailwind the authored form is a descendant selector on root (`**:[svg]:not-with-[size]:size-3.5`). Under the [owned-slot invariant](04-styles.md), the lift re-homes that measurement to the **icon slot** as its own `size` decl pinned on `{size, density}`. This is what lets StyleX — which has no descendant selector — reproduce per-density icon sizing: the icon slot merges its default size *before* any user-passed style, giving "default unless overridden" via merge order. Density-scaled icons work identically in both engines because the icon size is a first-class geometry cell on the slot that owns it.

**Icon padding stays root-owned, gated on a relation state.** The `iconPad` field ("extra edge padding when there's a leading/trailing icon") is *root's* padding — the declaration was always about root's own box. The lift keeps it a root-owned `paddingEnd`/`paddingStart` decl, but gates it on the `iconEnd`/`iconStart` relation state (driven by explicit `prefix`/`suffix` slot props, never children scanning). So `has-data-icon-end:pr-1.5` at compact `xs` is a `{size:'xs', density:'compact', state:'iconEnd'}` pin — density and relation composing cleanly. Both engines bind the same truth source.

The takeaway: density doesn't get its own special normalization. It rides the same owned-slot machinery every geometry declaration uses, which is exactly why a fourth tier (§10) needs no new plumbing.
...
flowchart TB
  subgraph system["System-wide (one choice per dsdoc)"]
    D["density axis<br/>role: 'density'<br/>compact / default / comfortable"]
  ...
  subgraph instance["Per-instance (props on one element)"]
    SZ["size prop<br/>role: 'size'<br/>xs / sm / md / lg"]
  end
...
| Dimension role | `density` | `size` | not a component dimension — token graph |
...
sequenceDiagram
  ...
  W->>W: refold affected component Contracts<br/>(density pin → new class maps)
  W-->>M: ClassMap deltas
...
The question this answers: *does the exported code carry the density dimension, or is the selected tier folded away?* Both engines support both modes.

### 10.1 `baked` (default)

The selected tier is resolved and its classes folded directly into the shipped component; the density dimension disappears from the output entirely. The user picked comfortable in the builder; the exported button ships comfortable geometry as plain classes, with no way to switch density at runtime. This is what most users want — they chose their density, they own code sized for it, and there's no dead dimension bloating the file.

**Tailwind, `baked`, comfortable md:**
```tsx
...
**StyleX, `baked`, comfortable md:**
```tsx
const styles = stylex.create({
  size_md: { height: tokens.spacing9, gap: tokens.spacing1_5, paddingInline: tokens.spacing2_5 },
  icon_size_md: { width: tokens.spacing4, height: tokens.spacing4 },
  // no density-keyed styles — the tier is folded in
})
```
...
The full density dimension ships as a `data-density` attribute axis; all three tiers' geometry lands in the exported code, and the consumer switches density at runtime by setting `data-density` on a container. This is for users who want density switching *in their own app* — a settings toggle, a compact "power user" mode.

**Tailwind, `runtime`:** density becomes a data-attribute variant, every tier present:
```tsx
const buttonVariants = tv({
  base: '…',
  variants: {
...
**StyleX, `runtime`:** density becomes a boolean-composed style keyed at the call site, all three tiers present:
```tsx
const styles = stylex.create({
  size_md_compact:     { height: tokens.spacing7, paddingInline: tokens.spacing2 },
  size_md_default:     { height: tokens.spacing8, gap: tokens.spacing1_5, paddingInline: tokens.spacing2_5 },
  size_md_comfortable: { height: tokens.spacing9, gap: tokens.spacing1_5, paddingInline: tokens.spacing2_5 },
})
// call site: styles[`size_${size}_${density}`]  — density read from context/prop
```
...
1. **Extend the dimension.** Add the new tier's slug to the density dimension's values in the [Registry Manifest](03-registry.md) baseline — e.g. `spacious` after `comfortable`. This is the one schema change, and it's one line.
...
3. **Rebuild the Contracts.** `pnpm build:registry` re-lifts: each new `sizes()` column folds into `{density:'spacious'}` pins on the Contract. No emitter, resolver, or normalization code changes — the new pin flows through exactly like the existing three.
4. **Expose it in the builder.** The density axis's `enum` control reads its options from the dimension values; the fourth option appears automatically. The tier classification (structural) is derived from the axis, so the new tier is correctly a structural-tier command with no wiring.
...
What does *not* change is the whole point: no producer, no `EngineEmitter`, no `resolve()`/`compile()` logic, no preview plumbing, no export mode. The [decision log](00-decision-log.md)'s framing — *density as hand-authored geometry tables* — is what makes this true: because a tier is data (a `sizes()` column), a fourth tier is more data.
...
- Density is a **Contract dimension** (`role: 'density'`), a peer of `variant`/`size`/booleans, flowing through `resolve()` and both engines with no special-casing.
...
- At **export**, `codeStyle.density` is **`baked`** (fold the selected tier, default, lean) or **`runtime`** (ship the full `data-density` axis, ~3× geometry, switchable) — both modes in both engines.
