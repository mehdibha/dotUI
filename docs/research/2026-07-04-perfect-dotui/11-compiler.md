...
The last stage turns resolved selections into complete [Style Contracts](04-styles.md) per sync group. The manifest ships each component's authored Contract with its named-style deltas — a named style like `button.style = 'brand-cta'` is authored as a DRY delta over the base Contract, but `resolve` materializes it into a *complete* Contract before anything downstream reads it. Three things happen:

1. **Named-style deltas → complete Contracts.** The selected named style's delta layers over the base Contract (base ← density-pin ← named-style delta), producing a complete, self-contained Contract with every rule present. Fork, diff, and export all operate on complete Contracts; the delta is an authoring convenience the resolver erases.

2. **User deltas applied.** The dsdoc's `components` section stores user overrides as *lifted Contract-delta JSON*, not raw class strings (the [dsdoc chapter](09-dsdoc.md) covers why: raw strings go through the same lift/validate at edit time, and only the lifted delta is stored). `resolve` applies these deltas over the resolved named-style Contract. Because they are already lifted, they carry the owned-slot invariant and both-engine totality with them — a user override cannot introduce a StyleX-inexpressible construct, because that was rejected at edit time.

3. **Scalar and param bindings resolved.** Component-contract scalar nodes (`--btn-radius`, `--btn-font-weight`) resolve to their bound targets from the axis selections. If the user bound Button's radius axis to `radius.lg`, the Contract's `--btn-radius` component var resolves to `{ token: 'radius.lg' }`. Whether that becomes an idiomatic `rounded-lg` class or a `var()` reference is a *compile*-time decision governed by `codeStyle.tokenIndirection` (§4.3) — `resolve` only records the resolved binding.

The result is a `ResolvedSystem`: the complete, engine-neutral, fully-materialized value from which every target compiles.

### The `ResolvedSystem` schema

```ts
interface ResolvedSystem {
  version: number                              // IR.version content hash (§6) — client/server handshake
  engine: 'tailwind' | 'stylex'               // the doc's selected engine
  ...
  // Per-sync-group resolved Style Contracts (complete, not deltas).
  contracts: Record<SyncGroupId, StyleContract>

  // Per-component resolution results.
  components: Record<ComponentId, {
    group: SyncGroupId                         // which Contract to read
    fileVariant: string                        // which base.tsx ships (loader: 'ring' → base.ring.tsx)
    scalarBindings: Record<string, TokenValue> // resolved component-var bindings ({ '--btn-radius': {token:'radius.lg'} })
...
Two properties of this schema carry the whole downstream design. First, **token values are symbolic, never stringified** — a semantic node holds `{ ref: 'accent', step: 500 }` or an OKLCH triple, not `"oklch(0.64 0.13 251)"` and never `"var(--color-primary)"`. Stringification is an emitter concern, so the same resolved value emits as a CSS custom property, a `stylex.defineVars` entry, or a DTCG `$value` from one source. Second, **contracts are complete and engine-neutral** — a `StyleContract` is slots × dimensions × states × token-typed declarations under the owned-slot invariant ([04-styles.md](04-styles.md)); it privileges neither engine, so choosing the emitter is genuinely the last step.
...
- **The `ResolvedSystem` is large.** A full system with 4 dimensions, 24 cells, ~76 semantic tokens, and ~72 components' resolved Contracts is a substantial object. The worker holds one, and the server allocates one per export request. The design accepts this memory cost in exchange for making `compile` a set of cheap projections; the alternative (streaming resolution lazily per target) would reintroduce the two-half-pipeline structure the whole design exists to delete.
- **Symbolic values defer all unit decisions to emit time.** A calc expression or an OKLCH mix is carried as a small AST, not a computed string. This is what makes multi-engine and DTCG fall out for free, but it means every emitter must implement the full `SymbolicValue` vocabulary — the catalog completeness test ([13-testing.md](13-testing.md)) exists precisely to prove both emitters are total over it, so a new `SymbolicValue` shape cannot ship with only one engine's rendering.
...
### 2.2 `export` → `RegistryFile[]` (EngineEmitter + codeStyle)

The export target produces the files a user installs: component sources, the theme stylesheet or tokens module, the utility library. It runs in two steps, both pure.

First, the selected engine's **EngineEmitter** (§3) walks the resolved Contracts and the resolved token graph to produce idiomatic source. The Tailwind emitter produces `tv()` literals whose class strings are *character-for-character* what the preview DOM rendered — same resolved values, same class order, so `bg-primary` in the preview is `bg-primary` in the file. The StyleX emitter produces `stylex.create` modules with boolean-composed styles. Crucially, the enum `vars` and the resolved scalar bindings flow through the *same* `ResolvedSystem` fields the preview read: the Contracts' `declaredVars` become emitted component CSS, scalar bindings become resolved classes or `var()` references per `tokenIndirection`. The axes the old exporter silently dropped — enum vars, global tokens, the radius-factor scalar — export with fidelity because they are in the `ResolvedSystem` and every target reads the same value.

Second, **codeStyle AST transforms** (§4) reshape the emitter's output to match the user's codebase conventions — arrow functions vs declarations, `tv()` class arrays vs one line per slot, import grouping, section comments. These are typed AST transforms over the emitter's parsed output, never regex over strings and never the old `// MARK:` in-band comment protocol. Every transform is AST-equivalent modulo formatting (CI invariant #10, [13-testing.md](13-testing.md)).
...
|---|---|---|---|
| `preview` | tokens (all cells) · contracts · components · engine | `PreviewOutput` (themeCss, utilitiesCss, runtimeVarsCss) | builder DesignScope · docs demos · live embeds |
| `export` | contracts · components · tokens · codeStyle · engine | `RegistryFile[]` (sources + theme/tokens + lib) | shadcn CLI · v0/Bolt/Lovable bundles · zip · export preview modal |
...
- **Four targets share `resolve` but each re-walks the resolved value.** `compile('export')` and `compile('preview')` both traverse every resolved Contract; they don't share a partial result. The design accepts this re-walk because each traversal is cheap object/string work over an already-resolved value (sub-millisecond per component), and sharing an intermediate would couple two targets that must stay independently correct.
- **`preview` and `export` must produce byte-identical *class strings* while producing different *wrappers*.** The preview wraps utility definitions the class strings reference; export wraps the class strings in a `tv()` literal. The invariant that the strings themselves are identical is exactly the live-variants conformance test (§5, [13-testing.md](13-testing.md)) — the one seam where a bug could reintroduce divergence, so it gets a property test *and* computed-style sample diffs, not trust.

---

## 3. The `EngineEmitter` interface

The engine-neutrality of the whole compiler funnels through one interface. Everything upstream — resolve, graph merge, ramp materialization, axis resolution, Contract resolution — is engine-neutral and computed once. The single point where Tailwind and StyleX diverge is the emitter, chosen per document by `resolved.engine`.

```ts
interface EngineEmitter {
  // THE one class/props-producing function — the seam that makes preview == export.
  slotStyles(contract: StyleContract, resolved: ResolvedSystem): SlotStyleMap

  emitPreview(resolved: ResolvedSystem, scope?: string): PreviewOutput
  emitExport(resolved: ResolvedSystem, codeStyle: CodeStyle): RegistryFile[]
  emitTokens(resolved: ResolvedSystem): { themeArtifact: string }  // CSS custom props | tokens.stylex.ts
}
```

Both implementations share responsibilities and split responsibilities cleanly.

**Shared, guaranteed by construction:** both consume the same resolved Contracts and the same symbolic token values, so the geometry (spacing, radius, OKLCH color) is identical before either engine runs — a StyleX button and a Tailwind button are pixel-identical because every value was resolved before the emitter was chosen. Both are *total* over the Contract vocabulary (the catalog completeness test proves every `PropKey × TokenValue × StateDecl` has both lowerings), so neither can silently drop a construct.

**Tailwind emitter responsibilities.** Re-collapse the Contract's normalized rules into idiomatic compact `tv()` configs — the owned-slot normalization that moved `**:[svg]:size-3.5` onto the icon slot is re-collapsed to the idiomatic `**:[svg]:not-with-[size]:size-3.5` peephole; the `iconEnd` relation state re-collapses to `has-data-icon-end:pr-2`; the pending complement expansion re-collapses to `pending:**:not-data-[slot=spinner]:opacity-0`. The output is byte-comparable to a hand-authored `styles.ts`. It emits `themeCss` as `@theme` semantics plus `:root` plus per-cell delta blocks with paired selectors. It honors `tokenIndirection`: where a contract node still points at its default target, it emits the idiomatic semantic utility (`bg-primary`); where a node is retargeted, it emits the `var()` form.

**StyleX emitter responsibilities.** Render the same rules as `stylex.create` + boolean composition — css-pseudo and rac-data states become nested condition objects; rac-render and relation states become boolean-keyed styles selected at the call site. It emits a `tokens.stylex.ts` via `stylex.defineVars` over the semantic vocabulary, with `createTheme` per class/data-bound mode option delta and media-bound options as native conditional values; explicit compound cells become compound themes documented to apply last. It emits `declaredVars` (the menu.highlight var-writes) so StyleX ships them exactly as Tailwind does. It relies on merge order for the icon-size guard: the default icon style merges before the user's, so StyleX's last-style-wins gives "default unless overridden" for free — no runtime children-scanning, because relation booleans come from explicit slot props (`prefix`/`suffix`).

The old publisher's template hole *was* a `tv()` call — `const <name>Variants = tv(%%TV_CONFIG%%)` with a forced `tailwind-variants` import — so supporting a second engine would have meant replacing the transform, the merge semantics, the serializer, and the template convention. Here, adding a third engine (vanilla-extract, CVA) is one more `EngineEmitter` implementation plus its rows in the catalog completeness test; nothing upstream moves.

### Tradeoffs (`EngineEmitter`)

- **StyleX cold start is heavier than Tailwind's unless precomputed.** Tailwind ships a static ~40KB utility layer; StyleX atomic rules are generated. The design precomputes a static atomic layer for the *base* document so StyleX cold start matches Tailwind's, and accepts a first-compile-in-worker cost only for a StyleX document that has already diverged from the base — a click-frequency cost, never a drag-frequency one.
- **The emitter is the largest correctness blast radius in the compiler.** A bug in `slotStyles` corrupts *both* preview and export for the affected engine at once, because they share that one function. The design accepts this concentration deliberately: one shared function with one conformance suite is safer than two hand-kept-in-sync code paths, which is exactly the runtime-`createStyles`-vs-publisher-`flatten` drift the old system lived with.

---

## 4. `codeStyle` — typed AST transforms

`codeStyle` is the second customization layer: not what the design system *looks like*, but what the exported *code* reads like. The old publisher's entire `codeStyle` axis was two booleans (`classArrays`, `sectionComments`) implemented as string surgery — `flattenClassArrays` space-joined arrays, `applySectionComments` regex-resolved `// MARK:` markers. Here `codeStyle` is a typed schema, and every option is an AST transform over the emitter's parsed output. The invariant (CI #10) is that every transform is AST-equivalent to the canonical source modulo formatting: a transform may reshape declarations and reorder imports, but it can never change program semantics.
...
interface CodeStyle {
  ...
  // StyleX-engine style
  sx: {
    varsModule: 'shared' | 'colocated'         // one tokens.stylex.ts  vs  vars beside each component
    conditionStyle: 'object' | 'separate-keys' // nested condition objects  vs  boolean-keyed styles
  }
...
| `sx.varsModule: 'colocated'` | inlines `stylex.defineVars` beside each component instead of importing a shared `tokens.stylex.ts` |
| `sx.conditionStyle: 'separate-keys'` | splits nested StyleX condition objects into boolean-keyed sibling styles |
| `comments.sectionSeparators: true` | inserts section-separator comments derived from the Contract's rule grouping (never `// MARK:` anchors) |
...
Take Button's resolved `primary` variant. Under the default `codeStyle` (`arrow`, `classArrays: true`, `flatten`, `baked`, default density), the Tailwind emitter produces:
...
- **The transform matrix multiplies with engines.** Every `codeStyle` transform must be correct for both Tailwind and StyleX output (or be engine-scoped, like `tv.*` and `sx.*`). This is more golden-test surface than a single-engine world, and CI #10 carries it — the design accepts the test cost because the alternative (an engine-agnostic string post-processor) is exactly the fragile `// MARK:` regex protocol the old system used.

---

## 5. Incrementality and caching

The compiler is fast because it caches aggressively, and it caches correctly because every cache is keyed on the *identity* of the node whose value it holds. Node identity is stable across edits (permanent readable ids, [05-tokens.md](05-tokens.md); stable rule ids, [04-styles.md](04-styles.md)), so a cache entry survives every edit that doesn't touch its node.
...
|---|---|---|
| Drag accent hue (seed change) | accent ramp's materialized cells (~2–4) | every other ramp, all Contracts, all axis resolutions |
| Retarget a semantic token | that node's resolved value | all ramps, all Contracts, all other tokens |
| Switch mode (light→dark) | *nothing* — `setAttribute`, cascade picks the delta block | the entire `ResolvedSystem` |
| Set `menu.highlight = accent` | menu's Contract resolution + runtimeVars | every other component, all tokens |
| Change Button's radius axis | Button group's Contract (scalar rebind) + ToggleButton, LinkButton (fan-out) | non-member components, all tokens |
| Add a mode dimension | stages 3–6 for the affected subgraph | Contracts not referencing the new dimension |
| Engine switch | `utilitiesCss`, all emitted output | the token graph, all resolved Contracts (engine-neutral) |
| Define a custom named style | that component group's Contract + incremental novel-utility compile | all tokens, all other components |

The two most-hit caches are ramp materialization (keyed `(rampSpecHash, cellKey)`) and Contract resolution (keyed `(syncGroupId, resolvedSelectionsHash)`). During a hue drag, the Contract cache is *entirely* untouched — no component's resolved selections changed — so `resolve` skips stage 6 completely and `compile('preview')` re-emits only `themeCss`. During a named-style change, exactly one group's Contract entry invalidates and the ramp cache is untouched.

The [builder](10-builder.md)'s tier classification is the operational face of this table: a **value**-tier edit hits only the token caches and emits via `setProperty`; a **structural**-tier edit invalidates one group's Contract; a **global**-tier edit re-runs the graph merge. The tier is derived from what the axis's command targets, so the invalidation and the tier agree by construction — a value edit *cannot* be misclassified as touching a Contract, because if it touched a Contract the classifier would have called it structural.
...
- **The client ships the full compiler.** A builder session downloads `resolve` + `compile` + both emitters (the active engine eagerly, the other lazily) plus the manifest baseline. This is larger than shipping a thin preview renderer. The design accepts it because the payload is the price of the structural preview==export guarantee, and it is bounded and cacheable.
...
- **Stable ordering everywhere.** Cells are emitted in a canonical order (dimension priority, then option order). Token nodes iterate by id, not by insertion. Contract rules emit in normalized order. Object key iteration is never relied on for output order; every collection that reaches output is sorted by a stable key first.
- **Canonical class order.** The Tailwind emitter emits utilities in a canonical order (the round-trip identity test, invariant #3 in [13-testing.md](13-testing.md), depends on it), so `bg-primary text-fg-on-primary` is never `text-fg-on-primary bg-primary` from one realm and the other. The order is a fixed function of the Contract's normalized rule structure, not of authoring order.
...
Determinism is what makes the cold-start parity test possible: the baked default `PreviewOutput` shipped with the builder must byte-equal a fresh server compile of the default document (invariant #5). If any stage were non-deterministic, that test could not pass, so the test is also the determinism guard.

---

## 8. Error taxonomy — typed diagnostics with source spans

Every failure the compiler can produce is a typed diagnostic, not a thrown string or a silent fallback. Diagnostics carry a machine code, a human message, and — where the failure traces to authored source — a source span produced by the lift ([04-styles.md](04-styles.md)). The same `validate()` that gates the builder's edit path and the CLI's `add`/`update` also produces these, so an agent editing a dsdoc through the MCP `validate` tool gets identical diagnostics to a human in the builder.
...
type Diagnostic = {
  ...
  span?: SourceSpan                             // file, line, column — from lift, when the failure is source-traceable
  fix?: ProposedEdit                             // a machine-applicable graph edit or Contract delta, when one exists
}
```
...
| Contract resolution | `STYLE_INEXPRESSIBLE` | a user delta introduces a construct with no both-engine lowering (caught at edit-time lift, surfaced here as defense-in-depth) |
...
- **Spans depend on the lift retaining source offsets.** A source span on a `STYLE_INEXPRESSIBLE` diagnostic is only as precise as the offsets the lift preserved. The design invests in span-carrying through the lift (so a bad class points at its exact character range) and accepts that graph-level diagnostics (a dangling token ref) point at a node id rather than a source range, because the dsdoc is JSON with no authored source to span into.

---

## 9. Worked example — Geist Button, edit to export

The [dsdoc chapter](09-dsdoc.md)'s worked document is Geist. Follow one axis through the whole compiler: a user forks the base doc, sets the engine to Tailwind, and drags the accent hue, sets `menu.highlight = accent`, then exports.

**Fork + resolve.** The forked dsdoc pins the current manifest. `resolve` runs: validate passes, no migration (same `dsdoc:` major), graph merge folds the empty overlay over the baseline (still ~76 semantic tokens, `scheme` + `contrast` dimensions), ramp materialization computes every ramp for the two default cells (`scheme:light`, `scheme:dark`) — accent, neutral, success, warning, danger, info — axis resolution applies the base defaults, Contract resolution produces complete Contracts for all ~72 components. The `ResolvedSystem` is warm.

**Drag the accent hue.** The builder classifies this as a **value**-tier edit (it targets a ramp seed). It sends the changed seed to the worker, which re-`resolve`s incrementally: only the accent ramp's `(accent, scheme:light)` and `(accent, scheme:dark)` cache entries invalidate; every other ramp and every Contract survive. Stage 6 is skipped entirely — no component's resolved selections changed. `compile('preview')` re-emits only `themeCss`; the ~22 changed custom properties (`--accent-50`…`--accent-950`, the `--on-accent-*` foregrounds) land as inline `setProperty` writes on the scope root. Every `bg-primary` in the showcase, and the menu's highlight, repaint via `var()` re-substitution. Zero React renders, zero sheet swaps — the 60fps trace.

**Set `menu.highlight = accent`.** A **structural**-tier edit. `resolve` re-runs stage 6 for the menu group only (<1ms); the resolved menu Contract now carries a `declaredVar` writing `--color-highlight: var(--accent-500)` and `--color-fg-on-highlight: var(--on-accent-500)`. `compile('preview')` emits these into `runtimeVarsCss`, `replaceSync`'d atomically. This is the exact field the old publisher dropped on export; here it is a `ResolvedSystem` field, so the next step cannot lose it.

**Export.** The user runs `npx shadcn init https://dotui.org/r/init?doc=<id>`. The `/r/*` server route calls the *identical* `compile(resolved, { kind: 'export', engine: 'tailwind', codeStyle })` — but first checks `IR.version`: the client that produced the doc ran the same compiler the server runs, so the handshake passes. The Tailwind `EngineEmitter` walks the resolved Contracts: Button's `primary` variant emits `bg-primary` (flatten, still-default target), the accent-derived color rides in the `theme.css` custom properties (both cells), and the menu's `runtimeVars` emit into the shipped menu CSS. codeStyle AST transforms shape the output to the user's conventions. The resulting `RegistryFile[]` carries class strings *character-for-character identical* to what the preview DOM showed — including the resolved `--color-highlight` the old system silently diverged on. Nothing previews that doesn't export.
...
- **`resolve`'s stages** consume the [Dimensional Token Graph](05-tokens.md) (stages 3–4), the [axis system](06-axes.md) (stage 5), and the [Style Contract](04-styles.md) (stage 6) — this chapter is where those three models become one materialized value.
...
- **The `EngineEmitter`** realizes the [two-engine](04-styles.md) promise: everything upstream is engine-neutral, and the emitter is the single, catalog-complete divergence point.

The compiler is the smallest possible surface that could satisfy the whole system: two pure functions, one resolved value between them, one bundle in two realms, one validator, one emitter interface. Every failure mode of the old publisher — dropped enum vars, dropped global tokens, hand-mirrored merge semantics, single-engine hardwiring, silent preset fallback, per-request mutable dep state — is a state this design cannot represent, because the value that flows through it is complete, symbolic, engine-neutral, and identical on both sides of the worker/server line.
