...
### P1 — Alpha / transparency ramps (Radix-style) — ✅ kernel shipped
**Status: shipped in the kernel** (`alpha: true` → `ThemeMode.alpha`, an `oklch(… / a)` twin per ramp via inverse alpha-compositing in sRGB against the mode background; opt-in, default output byte-identical). UI that blends into colored, image, or elevated backgrounds — selection, hover-on-card, overlays, borders over photos, glassy menus — needs alpha or it looks wrong on non-default surfaces. Radix ships a matching alpha variant per step (and a *separate* P3 alpha definition, since compositing differs by gamut). **A whole class of design systems disagree on solid-vs-alpha, so by the north star this is an axis** — now present in the engine.

**Remaining (www, not yet done):** emit `--{palette}-a{step}` vars + semantic overlay/translucent-surface tokens so it reaches the export; a dedicated P3 alpha definition (today's alpha is sRGB-computed, so it composites exactly on sRGB and closely on P3). Unblocks `CLAUDE.md`'s named "translucent menus/popovers" grouped switch, which previously had no alpha token to toggle.
