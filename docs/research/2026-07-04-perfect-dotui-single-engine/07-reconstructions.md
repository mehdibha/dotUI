...
A design-system builder either recreates real design systems or it does not. Every other chapter describes a mechanism — the [Dimensional Token Graph](05-tokens.md), the [axis system](06-axes.md), the [resolved styles](04-styles.md), [density](08-density-sizing.md), the [compiler](11-compiler.md). This chapter proves those mechanisms are sufficient by pointing them at five systems with sharply different identities and showing each one reconstructed as a complete [dsdoc](09-dsdoc.md) pinned to one [Registry Manifest](03-registry.md).

The five golden reference dsdocs live in the repo at `packages/registry/golden/{material3,geist,linear,enterprise,shadcn}.dotui.json`. They are not documentation samples — they are executable proof. CI validates them against the schema, resolves them against their pinned manifest, compiles each to Tailwind, and visual-regression-tests the rendered showcase every build. A look that cannot be reached is not a note in a design doc; it is a red test that names the axis the manifest lacks. That mechanism is described in full at the end (§7).

The claim under test is precise. **Every visual decision is a user-configurable axis of the builder, never a hardcoded choice.** The test for whether something is an axis is the constitution's test: *would two design systems disagree on it?* The five systems below disagree on nearly everything a design system can disagree on — surface strategy, radius, weight, elevation model, density, contrast, motion, and the exact names of tokens. Each disagreement resolves to an axis value, a token-graph override, or — where a system's component look genuinely diverges from every curated named style — a bounded set of component overrides. This chapter is honest about the last category: it lists every override each system needs, and there are never more than four.
...
- **`components`** carries **raw Tailwind class-string deltas** for the 2–4 looks per system that no curated named style reaches.

A reconstruction therefore stresses four layers at once. The **token graph** must express the system's color and dimension identity (literal Geist grays, Material tonal ramps, the enterprise square-corner factor). The **axes** must carry its shape/space/elevation/motion decisions without inventing tokens. The **named styles** must cover its component looks where possible. The **override** escape hatch must carry the rest — and stay small, or the axis catalog has a hole.
...
The **state layer** is the one mechanism that does not fit the plain fill model. It resolves through the `interaction.stateLayer` axis to `on-overlay`, whose vars emit hover/press as `color-mix(in oklab, var(--btn-fg) 8%, var(--btn-bg))` on the surface's `pairsWith` foreground — a `mix` token value, cell-resolved, verified as a composite over the surface.

### Component looks: named styles vs overrides

Material's default components map to named styles cleanly for **Button** (`filled` named style ≈ M3 filled button; `tonal`, `outlined`, `text` named styles cover the other M3 button variants) and **Card** (`elevated`/`filled`/`outlined` named styles align to M3's three card types). Two looks need override deltas:
...
```jsonc
{
  ...
  "components": {
    "fab":  { "delta": { "root": "rounded-[16px] bg-primary-container text-on-primary-container size-14" } },
    "chip": { "delta": { "root": "rounded-lg has-data-[icon-start]:pl-2", "close": "ml-auto" } }
  }
}
```
...
| Component fill | `button.fill` | override (see below) |
...
- **`button`** — override: the primary look is `bg-primary text-on-primary hover:opacity-90`, retargeting the base surface and using an opacity hover rather than a `-hover` token. This is a plain Tailwind class-string delta over the `primary` named-style param, and it fans out to **`toggle-button`** through the `button-like` sync group, so both land together (the constitution's sync rule).
...
Two overrides (one of which is a synced pair, so it is authored once). Because overrides are raw Tailwind now (decision **N3**), the opacity-hover and ring-flip strings paste straight through resolution and ship verbatim — no edit-time totality check stands between the pasted string and the exported file. This is the worked Geist dsdoc that recurs across the study; the full document appears in [the dsdoc chapter](09-dsdoc.md).
...
```jsonc
{
  ...
  "components": {
    "button": { "param": "primary", "delta": { "root": "bg-primary text-on-primary hover:opacity-90 pressed:opacity-80" } }
  },
...
- **`menu` / `popover`** — the translucent surface *plus* the specific 1px inner highlight border Linear draws at the top edge of overlays. The translucency itself is the fan-out axis; the top-edge highlight is an inline CSS-var write (`[--menu-highlight:...]` plus a `border-t` using it) that no named style carries, exactly the Menu-highlight fixture the study uses. This is one delta shared by the menu-like sync group.
...
Two overrides (the first synced across the menu-like group). The `--menu-highlight` write is exactly the case decision **N2** protects: because there is no strip step in resolution, the declared var flows into the shipped output verbatim, so the highlight ships. (The two-engine variant needed a `declaredVars` IR node to survive its lift; here the var is just Tailwind and never leaves the string.)
...
```jsonc
{
  ...
  "components": {
    "menu": { "delta": { "root": "[--menu-highlight:color-mix(in_oklab,white_8%,transparent)] border-t border-t-[var(--menu-highlight)]" } }
  }
}
```
...
- **`table` / `data-grid`** — enterprise data tables have zebra striping, a sticky bordered header, and per-cell dividers that the default table named style doesn't carry. A delta adds the row-striping and cell-border classes.
...
```jsonc
{
  ...
  "components": {
    "table": { "delta": { "row": "even:bg-elevated", "cell": "border-r border-border" } },
    "tag":   { "delta": { "root": "rounded-none border-l-[3px] border-l-[var(--tag-accent)]" } }
  }
}
```
...
One-to-two overrides, the lightest of the five — which is the point: shadcn is the system dotUI is most native to, and since dotUI now targets Tailwind alone, a shadcn override is quite literally a shadcn class string with nothing between it and the file.
...
```jsonc
{
  ...
  "components": {
    "button": { "param": "primary", "delta": { "root": "hover:bg-primary/90" } }
  },
...
| Opacity-hover flat button | Geist | override (button, synced) — raw Tailwind delta | global |
...
| Overlay top-edge highlight | Linear | inline `[--menu-highlight]` var write in override delta | global |
...
Every row lands on a declared axis, a token-graph construct, or a bounded override delta. Nothing lands on "hardcoded." The three tiers (value / structural / global) map each feature to how the [builder preview](10-builder.md) applies it — value-tier features are pure CSS-variable writes (60fps hue drags), structural pick a variant or file, global re-emits sheets.
...
Nine to ten distinct override deltas across five complete, recognizable design systems. That is the measure of the named-style catalog's coverage: the curated 20%-of-styles-that-cover-80% carries the vast majority of every system's components, and the escape hatch carries the genuinely idiosyncratic remainder — never more than a handful. Every override is now a **raw Tailwind class-string delta** over a named-style param (decision **N3**): it flows through the same resolution as authored styles, but there is no totality gate it must pass first, so even a genuinely exotic look — an arbitrary selector, a `peer-*` chain, a bespoke `[--x:…]` write — is expressible and ships. The escape hatch is wider than the two-engine variant's could be, precisely because nothing has to survive a lift.
...
3. **Compile + visual regression.** Each resolved system compiles to Tailwind, renders the standard showcase, and pixel-diffs against committed golden screenshots. A drift fails and links the diff image. Because the preview DOM and the exported file both come from the same `compile()`, this screenshot is a check on the shipped artifact, not a separate render path — the golden PNG is what a user who exports that dsdoc actually gets.
...
- **Raw overrides trade a totality guarantee for width.** Because overrides are raw Tailwind class-string deltas with no edit-time totality check (decision **N3**), a paste can be nonsense — a misspelled utility, a class that targets the wrong slot — and nothing catches it before the visual-regression screenshot. The golden PNG is the backstop, not a schema. This is a deliberate trade: it is what makes even exotic looks expressible, but it moves the "is this override correct?" question from a compile-time gate to review of the rendered result.

- **Golden screenshots are maintenance weight.** Visual-regression fixtures must be regenerated on every intentional baseline or manifest change, across every reachable cell of the multi-dimension systems (enterprise and M3 have four+ cells each). This is real reviewer burden — a legitimate baseline improvement produces a large, mostly-benign screenshot diff that a human must confirm is intended. The immutable-manifest pin bounds the churn (goldens only move when re-pinned, deliberately), but the cost is nonzero and recurring.
...
- **`tokenIndirection: 'preserve'` (shadcn) is a second emission path.** Serving shadcn's exact var names means the `tv()` emitter supports both the flattened semantic-utility form (default) and the preserved var form, and both must stay byte-correct. shadcn's whole value is name-fidelity, so this path cannot be dropped — but it is a permanent branch in the emitter that the shadcn golden alone guards.
