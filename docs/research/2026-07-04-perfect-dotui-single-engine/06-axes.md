...
function tierOf(axis: AxisDecl): "value" | "structural" | "global" {
  const w = axis.writes
  // GLOBAL: shape changes to the token graph — re-emit sheets.
  if (axis.kind === "tokenTarget" && addsOrRenamesNode(axis)) return "global"
  if (isDimensionAxis(axis)) return "global"
...
| **global** | add/rename token, add mode or dimension, custom style | re-emit sheets — full recompile in the worker |
...
Component axes are **synthesized from the group's [contract declarations](04-styles.md)**, but **exposure is curated** — internal mechanics vars never become axes; only the curated 20%-that-covers-80% named styles and knobs surface. All style axes on a group are sync-group-scoped (they hit every member).
...
`codeStyle` axes shape the *exported code*, not the rendered look. They are `global` tier (they change emitted files, not the preview surface) and are AST transforms over `tv()` emitter output — never regex over comment anchors (see [chapter 11](11-compiler.md)).
...
There is no `engine` axis. dotUI targets one engine — idiomatic Tailwind — so the export target is never a design choice; every packaging (§ [chapter 12](12-distribution.md)) reads the one `ResolvedSystem` and the `tv()` emitter is the only emitter. (The two-engine study carried a top-level `engine` enum here whose values selected between two emitters; with a single engine that decision, and the preview machinery it demanded, simply does not exist.)
...
- **Curated exposure of component axes is a judgment call, not a rule the type system enforces.** "The 20% that covers 80%" is chosen per group by contributors; a genuinely-needed named style that wasn't curated is reachable only through the advanced `ComponentOverride` surface ([chapter 04](04-styles.md)) — now a raw Tailwind class-string delta, which is heavier but arbitrary. This is the honest cost of not auto-exposing every contract node as an axis — which would drown the panel in internal mechanics knobs.
