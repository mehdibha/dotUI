...
This chapter is the whole of `/create`: the information architecture, the generated panel, the command store, the Style Worker protocol, the DesignScope preview mechanism, the update-tier pipeline with its frame budgets, cold start, docs and embed reuse, the complete user journeys, and AI assist.
...
```
┌──────────────────────────────────── /create ─────────────────────────────────────────┐
│ TopBar   ⌘K search · system name · Light/Dark/HC mode · Share · Export                │
├───────────────┬───────────────────────────────────────────────────┬───────────────────┤
│               │                                                   │                   │
...
type Command =
  ...
  | { t: 'component.override'; component: ComponentId; delta: ClassStringDelta }      // raw Tailwind
  | { t: 'codeStyle.set';     patch: Partial<CodeStyle> }
  | { t: 'batch';             label: string; cmds: Command[] }   // one undo step
```

The override command carries a **raw Tailwind class-string delta** ([decision N3](00-decision-log.md)) over a named-style param — not an engine-neutral rule object. With one engine there is nothing to translate and no edit-time totality check to run, so the user may paste arbitrary Tailwind (`peer-*`, `:has()`, descendant combinators, arbitrary values) and it flows through resolution and ships. There is no `engine.set` command — the union has no engine axis to switch.
...
| **global** | the token/utility universe: add/rename token, add mode/dimension, custom style | worker full recompile (+ incremental Tailwind WASM if new utilities) → atomic three-sheet replace | ~5–20ms, click-frequency | scoped |
...
// worker-resident state
{
  ...
  resolved: ResolvedSystem,       // current fully-resolved system (per-cell token values, per-group ResolvedComponentStyles)
...
There is no `engine` field on the worker state. The worker resolves and emits Tailwind — the only engine.
...
- **`compileExport`** runs `compile(resolved, { kind: 'export', codeStyle })` in the worker so the code-style modal previews the *actual served bytes*, instantly, offline. There is no engine parameter — one `compile`, one target family.
...
1. **Three adopted stylesheets** on the scope, via `adoptedStyleSheets`:
   ...
   - **`utilitiesSheet`** — the Tailwind utility layer: the static precomputed utility rules, appended incrementally on new-utility events.
...
With no intermediate representation and no cross-engine parity gate, **`createLiveVariants` is the single fidelity seam in the whole system** — the only place the preview code and the export code differ. Everything else on the stage is the literal `base.tsx` running the literal resolved `tv()` config. So the entire "what you see is what you own" guarantee reduces to one property test plus one computed-style diff over this shim (§ testing, [chapter 13](13-testing.md)). It is the load-bearing invariant precisely because nothing else stands between preview and export.
...
Because there is one engine, cold start is one story: adopt the baked Tailwind sheets, boot the worker, swap. The two-engine study carried an extra precomputed atomic layer to keep a second engine's cold start on par; this design has nothing to keep on par, so that machinery — and the "which engine does the preview run" question with it — does not exist.

### 6.4 Docs-page preview reuse
...
With a system in place, the user drives the panel: enum styles per component, per-component radius, the translucency fan-out switch, a custom named style for the primary button (a `component.override` carrying a raw Tailwind class-string delta — global tier, the incremental Tailwind pass compiles any novel utility). Arbitrary Tailwind is accepted verbatim; there is no edit-time totality check to satisfy. The family editor keeps synced groups in step; detach chips handle the one intentional exception.
