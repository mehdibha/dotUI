...
The dsdoc is deliberately thin. It does not carry dotUI's vocabulary — the [axes](06-axes.md), the baseline [token graph](05-tokens.md), the per-group [resolved styles](04-styles.md), the component roster. That vocabulary lives in the **Registry Manifest**, an immutable snapshot the dsdoc pins by version. The dsdoc stores only what *differs* from that pinned baseline: an overlay on the token graph, a handful of axis selections, the rare component style delta, the code style. A share that changes only the accent hue and the corner radius is a few hundred bytes. This is the spine of the whole design — **declaration versus selection**. The manifest declares *what can be chosen and how it renders*; the dsdoc records *what was chosen*. Minimal diffs, tiny shares, and a computable update story all fall out of that split.
...
interface Dsdoc {
  ...
  components?: Record<ComponentId, ComponentDelta>  // user class-string deltas (§6)
...
Eleven keys, and every one of them is either identity, a pin, an overlay, or a set of selections. There is no `engine` field — dotUI targets exactly one style engine (idiomatic Tailwind), so there is nothing to choose. There is no `primitives`, no `modes`, no separate `tokens`-vs-`semantics` split — the constitution collapses the config-axes judge's `primitives`/`modes`/`tokens` trio into a **single `tokens: TokenGraphOverlay`** because the Dimensional Token Graph is the one model that already carries ramps (primitives), mode dimensions, and semantic and component nodes as one structure. The dsdoc overlays that structure; it does not re-describe it in three shapes.
...
The consequence is that **a two-year-old document opens frozen**. Load a dsdoc pinned to `2026.11.20-7c2` and the [resolver](11-compiler.md) fetches *that* manifest — its axes, its defaults, its resolved styles, its token baseline — exactly as they were the day it was authored. dotUI can add axes, retune baseline tokens, redesign the default button, and ship a hundred new manifest versions in the interim; none of it touches the frozen document. Untouched sliders cannot move under the user's feet, ever, because "untouched" resolves against a snapshot that cannot change.
...
interface TokenNodeDelta {
  ...
  values?: Record<CellKey, NodeValue>    // '' base + per-cell overrides; retargets any existing node
...
## 6. Components — raw Tailwind class-string deltas

When a user's desired look exceeds any curated axis value — Geist's flat black button, an enterprise square outline — the dsdoc carries the style itself. With a single engine, it carries **exactly what ships: raw Tailwind class-string deltas** over the named-style params of the affected sync group.

```typescript
interface ComponentDelta {
  classes?: ClassStringDelta             // raw Tailwind deltas keyed by tv() section path (see 04-styles.md)
  variants?: Record<string, string[]>    // add/remove instance-prop vocabulary
}

// Deltas are keyed by tv() section path: base / a slot / a variant value / a compound.
interface ClassStringDelta {
  base?: string
  slots?: Record<SlotId, string>
  variants?: Record<string, Record<string, string>>   // variant name → value → class string
  compounds?: Array<{ when: Record<string, string>; class: string }>
}
```

This is the D3 flip, and it matters. A user authors an override two ways — through the builder's visual style editor, or by pasting a Tailwind class string directly into a style input — and **both land as raw class strings** keyed by the `tv()` section they patch. There is no lift, no edit-time totality check, and no intermediate JSON: the delta *is* the utility string, and it flows through [style resolution](04-styles.md) into the emitted `tv()` config exactly as registry-authored classes do. Because the engine is unconditional, whatever the user pastes is guaranteed-shippable Tailwind by construction — there is no second engine that could reject it, so **arbitrary Tailwind is accepted**: `:has()`, `peer-*`, descendant combinators, container queries, arbitrary values, all first-class.

Storing raw class strings — rather than a lifted, engine-neutral delta — buys three things and costs one:

- **Widest possible override surface.** Any Tailwind a contributor could author in `styles.ts`, a user can paste into the builder. There is no whitelist ceiling, no closed authoring vocabulary, no declaration that could fail to lower. This *widens* the "recreate any design system" promise rather than bounding it. (The two-engine study bounded this surface to what both engines could lower — a constraint this design does not carry.)
- **The override participates in resolution by section.** A class-string delta merges onto the resolved `tv()` config for the group at a named section path (base / slot / variant / compound), so the resolver still reasons about *where* the override lands even though the value itself is an opaque string.
- **Code style applies.** Because the override is data keyed to `tv()` sections, the [compiler's](11-compiler.md) `codeStyle` AST transforms project over it exactly as they do over registry-authored classes — the exported override reads in the user's own taste, same one-line-per-variant / class-array shape as the rest.
- **The cost: no cell-by-cell reasoning.** A raw class string is opaque *within* a section — the resolver cannot decompose `bg-black text-white hover:opacity-90` into typed declarations. The hardcoded-value lint (a warning, not a gate) still flags a design-meaningful literal like `bg-[#000]` with a token hint, but it does not block the delta from shipping. Targeting is by `tv()` section path + named-style id, which is enough for merge and diff.
...
Here is a real, complete Geist document, in the constitution's `tokens: TokenGraphOverlay` shape. Every section is doing exactly one job.
...
```jsonc
{
  ...
  "components": {
    "button": {                                       // no curated value = Geist-flat: a raw class-string delta
      "classes": {
        "variants": {
          "fill": {
            "solid": "bg-primary text-primary-fg hover:opacity-90"  // replace the solid surface layer
          }
        }
      }
    }
...
| The flat button look (beyond curated values) | `components.button.classes` — a raw Tailwind class-string delta |
...
Note what is *absent*: no `engine` field — one engine is implied. No `primitives`, `modes`, and `tokens` as three sections — one `tokens` overlay. No lifted JSON in `components` — a raw Tailwind class-string delta, the same utilities that ship. No `axes` — Geist declares no new knobs, only selects existing ones. The document is small because declaration lives in the pinned manifest and the dsdoc records only Geist's differences.
...
- **Raw class-string deltas are opaque within a section.** Storing the utility string, not a typed structure, is what makes "paste any Tailwind" work — but it also means the resolver cannot reason *inside* a delta cell by cell, so the hardcoded-value lint can only warn, and a diff of two overrides is a string diff rather than a structured one. The override surface is maximally wide; the cost is that the resolver treats each section's classes as an opaque blob.
