...
| [004](004-docs-section-gallery-nav.md) | Charts page + nav | DONE — **superseded by a restructure (maintainer feedback)**: the multi-page docs section was replaced by a single creative standalone page at `/charts` (route `_app/charts.tsx`, module `modules/charts/`) — hero + live gallery of all 6 families + an interactive playground (family tabs, every variant, live preview, instant `--chart-N` palette presets, `?raw`+shiki source view, install command). `content/docs/charts/*` and the docs-sidebar "Charts" section were removed; the `llms.txt` bucket + components-overview block reverted. The `componentsData` charts category + `/create` builder integration are retained. (Original plan — Charts sidebar section + `meta.json`, gallery `overview.mdx` + 6 family pages, `componentsData` category, gallery card previews, navbar link, `llms.txt` third bucket, Components-overview block.) |
