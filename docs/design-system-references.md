# Design system references

The systems worth studying when researching presets, missing axes, or component comparisons. Link-verified July 2026.


## Headless libraries

- [React Aria Components](https://react-spectrum.adobe.com/react-aria/)
- [Base UI](https://base-ui.com/react/overview/about)
- [Radix Primitives](https://www.radix-ui.com/primitives)
- [Ark UI](https://ark-ui.com/)

## Component libraries

- [shadcn/ui](https://github.com/shadcn-ui/ui)
- [HeroUI](https://github.com/heroui-inc/heroui)
- [coss ui](https://coss.com/ui)
- [Radix Themes](https://github.com/radix-ui/themes)

## Design systems

- [Spectrum 2 (Adobe)](https://github.com/adobe/spectrum-design-data)
- [Astryx (Meta)](https://github.com/facebook/astryx)
- [Material 3](https://github.com/material-foundation/material-color-utilities)
- [Geist (Vercel)](https://vercel.com/geist/introduction)
- [Primer (GitHub)](https://github.com/primer/primitives)
- [Supabase](https://supabase.com/design-system)
- [Atlassian](https://atlassian.design/foundations/tokens/use-tokens-in-code/)
- [Cloudscape (AWS)](https://cloudscape.design/)
- [Carbon (IBM)](https://github.com/carbon-design-system/carbon)

### Colors

- [Radix Colors](https://github.com/radix-ui/colors)
- [Leonardo (Adobe)](https://leonardocolor.io/)
- [material-color-utilities](https://github.com/material-foundation/material-color-utilities)

## Inspiration

### Preset customizer

- [shadcn/create](https://ui.shadcn.com/create)
- [HeroUI Pro Theme Editor](https://heroui.pro/ds)
- [Astryx Playground](https://astryx.atmeta.com/playground)

### Preset targets

- [Linear](https://linear.app/brand)
- [Geist (Vercel)](https://vercel.com/geist/introduction)
- [Stripe](https://docs.stripe.com/stripe-apps/style)
- [Claude / Anthropic](https://www.anthropic.com/)
- [Supabase](https://supabase.com/design-system)
- [Raycast Themes](https://ray.so/themes)

### Components catalog

- [HeroUI pro](https://heroui.pro)
- [Beautiful UI (Turbo)](https://beautiful-ui-five.vercel.app/) — AI-native UI primitives

### Motion & interaction

- [interior.dev](https://www.interior.dev/docs) — copy-paste headless interaction primitives (layout-shift-free state changes, interruptible animations, reduced-motion)

### Button

- [Supabase](https://supabase.com/design-system/docs/components/button)

### Charts

### Command Dialog

