...
type Icon = { colorType: "fill" | "stroke"; name: string; icon: IconNode };

export default function IconsList(props: IconsListProps) {
  const { className, libraries } = props;

  const [allIcons, setAllIcons] = React.useState<Icon[]>([]);
  ...
  React.useEffect(() => {
    const loadIcons = async () => {
      ...
      if (libraries.includes("lucide-icons")) {
        ...
        loadedIcons = [
          ...loadedIcons,
          ...lucideIcons.map((icon) => ({ ...icon, colorType: "stroke" })),
        ] as Icon[];
      }
      if (libraries.includes("simple-icons")) {
        ...
        loadedIcons = [
          ...loadedIcons,
          ...simpleIcons.map((icon) => ({ ...icon, colorType: "fill" })),
        ] as Icon[];
      }
                ...
                >
                  {allIcons.length > 0
                    ? icons
                        ...
                        .map((icon, index) => {
                          const Icon = createIcon(icon.name, icon.icon, icon.colorType);
                          const code = createIconComponentString(
                            icon.name,
                            icon.icon,
                            icon.colorType
                          );
                          return (
                            <IconWrapper key={index} name={icon.name} code={code}>
                              <Icon />
                            </IconWrapper>
                          );
                        })
