...
- **Known-weak:** the components page (several demos misbehave); the built-in presets' fidelity — they are now frozen, revisioned snapshots (Origin's latest revision is the axis defaults) listed in the studio's picker (`/presets` redirects there), the landing and the docs preview picker (which defaults to the reader's studio system), but their looks are due to be reworked into high-fidelity recreations of popular systems (Linear, Vercel, …); chart colors (need a full rewrite); site performance overall (never audited — a docs site must feel instant).
- **The /studio panel** (`www/src/modules/studio/`): the drill-in panel over the studio state. `axes/<chapter>.ts` owns each chapter's state slice, option vocabularies and `resolve(state)` — the pure mapping to what the engine consumes (global tokens, registry enum params, density, the color recipe, the icon library); `resolve.ts` composes them into the `DesignSystem` the provider renders, the preview iframe receives, and `/r/*` publishes. `sections/<chapter>.tsx` is the chapter's UI only (option lists come from the axis module). Adding an axis = a key in the axis module's defaults + a mechanism the registry consumes (a token read by base.css / a `-(--var)` utility, or an enum param with tv slices in `styles.ts`); default selections must flatten to today's look, and exported code must never carry two conflicting classes. Per-component live-tweak vars (`ui/<component>/styles.css` `:root`, `roles.css`) are `--studio-*`: builder-only, resolved to plain utilities on publish, never shipped — the publisher fails on any survivor. Panel chrome is instant by design — no easing on overlay open/close or pane swaps (in-place control feedback like segmented indicators still eases).
