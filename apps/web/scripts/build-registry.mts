...
async function createTempSourceFile(filename: string) {
  const dir = await fs.mkdtemp(path.join(tmpdir(), "dotui-"));
  return path.join(dir, filename);
}

// ----------------------------------------------------------------------------
// Build registry/demos.tsx.
// ----------------------------------------------------------------------------
...
`;
  ...
  const components = readdirSync(path.join(process.cwd(), "src/registry/ui/default/core"));
  for (const component of components) {
    if (!existsSync(path.join(process.cwd(), "src/registry/ui/default/core", component, "demos"))) {
      continue;
    }
    const demos = readdirSync(
      path.join(process.cwd(), "src/registry/ui/default/core", component, "demos")
    );
  ...
  for (const style of styles) {
    ...
    for (const item of registry) {
      const resolveFiles = item.files?.map(
        (file) => `registry/ui/${style.name}/${typeof file === "string" ? file : file.path}`
      );
      if (!resolveFiles) {
        continue;
      }
        ...
        chunks = await Promise.all(
          components.map(async (component, index) => {
            ...
            const targetFilePath = path.join(
              cwd(),
              `registry/ui/${style.name}/${type}/${chunkName}.tsx`
            );
            ...
            return {
              name: chunkName,
              description,
              component: `React.lazy(() => import("@/registry/ui/${style.name}/${type}/${chunkName}")),`,
              file: targetFile,
              container: {
...
`;
  ...
  const items = registry
    .filter((item) => ["registry:core"].includes(item.type))
    .map((item) => {
      return {
        ...item,
        ...
        files = await Promise.all(
          item.files.map(async (_file) => {
            ...
            const content = await fs.readFile(
              path.join(process.cwd(), "src", "registry", "ui", style.name, file.path),
              "utf8"
            );
  ...
  for (const style of styles) {
    ...
    const payload: RegistryEntry = {
      ...
      dependencies: [
        ...
        "lucide-react",
        // TODO: Make this custom from components json config
