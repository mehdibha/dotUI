...
import { RADIUS_TOKENS, SCALE_STEPS } from "./constants";
import type {
  ColorFormat,
  ColorScale,
  ColorTokens,
...
export const toColorString = (colorValue: string): CssColor => {
...
export const convertColorFormat = (
  ...
  try {
    ...
    if (targetFormat === "hsl") {
      // Use culori for HSL conversion
      const hsl = converter("hsl");
      const converted = hsl(hexColor);
      if (converted) {
        const h = converted.h ? Math.round(converted.h) : 0;
        const s = Math.round((converted.s ?? 0) * 100);
        const l = Math.round((converted.l ?? 0) * 100);
        return `hsl(${h} ${s}% ${l}%)`;
      }
    }
  } catch (error) {
