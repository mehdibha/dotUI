...
const IMPORT_TRANSFORMATIONS: ImportTransformation[] = [
  // Transform component imports to remove variants - @/components/button/basic → @/components/button
  {
...
async function setupTargetDirectories(config: BuildConfig): Promise<void> {
  ...
  if (existsSync(targetDir)) {
    ...
    console.log(`🗑️  Cleaned existing registry`);
  }
...
async function copyDirectoryWithTransform(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      console.log(`✓ Built: ${path.relative(sourceDir, sourcePath)}`);
    }
  }
}
...
async function buildRegistry(config: BuildConfig): Promise<void> {
  ...
  console.log(`🔄 Building registry [${foldersToSync.join(", ")}]`);
  ...
  try {
    ...
    console.log(`✅ Successfully built registry to UI package`);
  } catch (error) {
