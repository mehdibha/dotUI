...
const radixFamilies = Object.keys(radix).filter(
  (k) => !/Dark|A$|P3/.test(k) && !RADIX_GRAYS.has(k) && !/^(black|white)/.test(k),
);
...
function measureScale(name, entries) {
  ...
  return { name, steps, peakStep: peak.step, peakC: peak.c, peakH: peak.h, cusp: familyCusp };
}
...
const twAll = Object.entries(twScales).map(([fam, steps]) => {
    ...
    return { name: fam, steps: measured, peakStep: peak.step, peakC: peak.c, peakH: peak.h, cusp: familyCusp, outOfSrgbSteps: outOfSrgb };
  });
...
function collapseStats(data, nSteps) {
  ...
  for (let i = 0; i < nSteps; i++) {
    ...
    const s = (arr) => {
      ...
      return { mean: round(mean), sd: round(sd), cv: mean > 0 ? round(sd / mean, 3) : 0 };
    };
    stats.push({ step: i + 1, absC: s(abs), cNorm: s(norm), cRel: s(rel) });
  }
  const avg = (key) =>
    round(stats.reduce((a, b) => a + b[key].cv, 0) / stats.length, 3);
  return { perStep: stats, meanCV: { absC: avg("absC"), cNorm: avg("cNorm"), cRel: avg("cRel") } };
}
...
for (const s of radixVividStats.perStep)
  console.log(`  step ${String(s.step).padStart(2)}: cRel ${s.cRel.mean} ± ${s.cRel.sd} (cv ${s.cRel.cv}) | cNorm ${s.cNorm.mean} ± ${s.cNorm.sd} (cv ${s.cNorm.cv})`);

console.log("\nTailwind neutrals excluded:", twNeutrals.map((f) => f.name).join(" "));
...
for (const s of twStats.perStep)
  console.log(`  idx ${String(s.step).padStart(2)}: cRel ${s.cRel.mean} ± ${s.cRel.sd} (cv ${s.cRel.cv}) | cNorm ${s.cNorm.mean} ± ${s.cNorm.sd} (cv ${s.cNorm.cv})`);
