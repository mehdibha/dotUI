...
const ratioSummary = (i) => {
  ...
  return { step: i + 1, median: r3(median(rs)), min: r3(Math.min(...rs)), max: r3(Math.max(...rs)) };
};
  ...
  JSON.stringify(
    {
      ...
      ratioSummaryPerStep: Array.from({ length: 12 }, (_, i) => ratioSummary(i)),
...
const stats = (key) => {
  ...
  return { min: r3(Math.min(...vals)), median: r3(median(vals)), max: r3(Math.max(...vals)) };
};
const below = (key, bar) => audit.filter((a) => a[key] < bar).map((a) => a.family);
  ...
  JSON.stringify(
    {
      meta: {
        ...
        meters: "apca-w3 APCAcontrast+sRGBtoY (abs Lc); WCAG2 relative-luminance ratio",
      ...
      lightSummary: {
        ...
        below_Lc90_step12: auditLight.filter((a) => a.lc12on2 < 90).map((a) => a.family),
        below_Lc60_step11: auditLight.filter((a) => a.lc11on2 < 60).map((a) => a.family),
        below_wcag45_step11: auditLight.filter((a) => a.wcag11on2 < 4.5).map((a) => a.family),
        below_wcag7_step12: auditLight.filter((a) => a.wcag12on2 < 7).map((a) => a.family),
      },
      perFamilyDark: audit,
...
console.log(`families: ${families.length} (${chromatics.length} chromatic + ${GRAYS.size} grays)\n`);
console.log("DARK  median L* 1-12:", fmt(skeleton.dark, "median"));
console.log("LIGHT median L* 1-12:", fmt(skeleton.light, "median"));
console.log("\nDARK  ΔL* between medians 1→8:", deltas(skeleton.dark).map((d) => d.dL.toFixed(1)).join("  "));
console.log("LIGHT ΔL* between medians 1→8:", deltas(skeleton.light).map((d) => d.dL.toFixed(1)).join("  "));
console.log("\nstep-1 dark L*: min", Math.min(...step1.map((s) => s.dark_step1_Lstar)).toFixed(2),
  "max", Math.max(...step1.map((s) => s.dark_step1_Lstar)).toFixed(2),
  "median", median(step1.map((s) => s.dark_step1_Lstar)).toFixed(2));
console.log("Δ(1→2) dark median:", median(step1.map((s) => s.dark_d12)).toFixed(2),
  "| light median:", median(step1.map((s) => s.light_d12)).toFixed(2));
...
for (const f of ["yellow", "amber", "orange", "lime", "mint", "sky"]) {
  ...
  console.log(` ${f}: 11 ${row.text.C11_light}→${row.text.C11_dark} (x${row.text.ratio11})  12 ${row.text.C12_light}→${row.text.C12_dark} (x${row.text.ratio12})`);
}
console.log("\naudit: step12 on step2 —", JSON.stringify(stats("lc12on2")),
  "| below Lc90:", below("lc12on2", 90).length + "/31");
console.log("audit: step11 on step2 —", JSON.stringify(stats("lc11on2")),
  "| below Lc60:", below("lc11on2", 60).length + "/31");
console.log("below WCAG 7.0 (12):", below("wcag12on2", 7).length, below("wcag12on2", 7).join(","));
console.log("below WCAG 4.5 (11):", below("wcag11on2", 4.5).length, below("wcag11on2", 4.5).join(","));
