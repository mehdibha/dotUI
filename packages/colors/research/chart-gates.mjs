...
function viridisTable() {
  ...
  for (const [idx, hex] of quartiles) {
    ...
    if (d > 0.02)
      throw new Error(
        `viridis[${idx}] = ${colors[idx]} too far from ${hex} (ΔEok ${d})`,
      )
  }
  return colors
}
...
function measureCategorical(entries) {
  ...
  for (const [cvd, filter] of Object.entries(cvdFilters)) {
    result.minPairwiseDeltaEok[cvd] = minPairwise(
      parsed.map((c) => filter(c)),
      names,
    )
  }
...
//   grade uniformity.
const normalRef = Math.min(
  tab.minPairwiseDeltaEok.normal.min,
  okabe.minPairwiseDeltaEok.normal.min,
)
...
const gates = {
  categorical: {
    ...
    derivedFrom: {
      ...
      referenceLstarRanges: {
        tab10: tab.lStar.range,
        okabeIto7noBlack: okabeNoBlack.lStar.range,
      },
...
function testCategorical(m, g) {
  ...
  checks.lStarAdjacentStagger = {
    ...
    pass:
      m.lStar.minAdjacentInOrderGap >=
      g.lStar.generationRule.minAdjacentInOrderDeltaLstar,
  }
