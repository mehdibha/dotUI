...
const data = JSON.parse(
  readFileSync(new URL("./data/radix-light-chroma.json", import.meta.url), "utf8"),
);
...
for (let mu = 8.0; mu <= 11.0; mu += 0.05) {
  for (let sL = 2.0; sL <= 6.0; sL += 0.05) {
    for (let sR = 1.0; sR <= 4.0; sR += 0.05) {
      ...
      for (const f of vivid)
        for (let i = 0; i < N; i++)
          sse += (f.peakC * skewGauss(i + 1, [amp, mu, sL, sR]) - f.steps[i].c) ** 2;
...
const peakStepCount = {};
for (const f of vivid) peakStepCount[f.peakStep] = (peakStepCount[f.peakStep] ?? 0) + 1;

const out = {
  meta: {
    generated: "d5-chroma-fit.mjs",
    families: { radixVivid: vivid.map((f) => f.name), excludedMuted: [...MUTED] },
    residualUnits:
...
console.log("\ng skew-Gaussian:", out.parametric.gSkewGaussian.params, out.parametric.gSkewGaussian.fit);
...
console.log("TW k quadratic:", out.parametric.tailwindKQuadratic.params, out.parametric.tailwindKQuadratic.fit);
console.log("TW k table:", out.parametric.tailwindKQuadratic.table.join(", "));
console.log("\nWinner worst residuals:", JSON.stringify(out.winnerDiagnostics.worstResiduals));
