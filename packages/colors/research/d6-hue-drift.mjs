...
function fitSlope(points) {
  ...
  return {
    slopePerL: round(k, 2),
    r2: ssTot === 0 ? 1 : round(1 - ssRes / ssTot, 3),
    n: points.length,
  }
}
...
function analyzeScale(steps, refStep) {
  ...
  return {
    ...
    dark: {
      ...
      endpoint: darkEnd
        ? { step: darkEnd.step, L: darkEnd.L, C: darkEnd.C, dH: darkEnd.dH }
        : null,
    },
    light: {
      fit: fitSlope(lightSide.map((s) => ({ x: s.L - refL, y: s.dH }))),
      endpoint: lightEnd
        ? { step: lightEnd.step, L: lightEnd.L, C: lightEnd.C, dH: lightEnd.dH }
        : null,
    },
  }
}
  ...
  JSON.stringify(
    {
      ...
      radix: radixOut,
      tailwind: twOut,
      tailwindNeutralsExcluded: twNeutrals,
    },
    null,
    2,
  ),
