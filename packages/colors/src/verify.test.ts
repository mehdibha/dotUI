...
describe('verify', () => {
  ...
  it('size-awareness: the same pairing has a lower target for large/nonText than body', () => {
    const body = verify([
      { name: 'x', fg: '#767676', bg: '#fff', sizeClass: 'body' },
    ]).results[0]!
    const large = verify([
      { name: 'x', fg: '#767676', bg: '#fff', sizeClass: 'large' },
    ]).results[0]!
  ...
  it("verifyTheme: an oklch theme's on-* contract passes (WCAG2)", () => {
    const theme = createTheme({
      algorithm: 'oklch',
      palettes: { primary: '#3b82f6', success: true },
    })
  ...
  it('verifyTheme(apca) reports honest failures (floor unreachable on mid steps)', () => {
    ...
    expect(report.passed).toBe(false)
    expect(
      report.failures.every((f) => f.formula === 'apca' && f.target === 60),
    ).toBe(true)
  })
