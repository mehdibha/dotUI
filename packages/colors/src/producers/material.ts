...
function tonalPalette(seed: string, neutral: boolean): TonalPalette {
  ...
  const hct = Hct.fromInt(
    argbFromRgb(Math.round(r * 255), Math.round(g * 255), Math.round(b * 255)),
  )
  return TonalPalette.fromHueAndChroma(
    hct.hue,
    neutral ? NEUTRAL_CHROMA : hct.chroma,
  )
}
...
export const materialProducer: ColorProducer<MaterialOpts> = {
  ...
  produce(opts, ctx) {
    ...
    const tones = resample(
      opts.tones ?? (ctx.isDark ? DARK_TONES : LIGHT_TONES),
      ctx.steps.length,
    )
