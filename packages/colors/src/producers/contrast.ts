...
import { gamutMap, makeContrast, type Oklch, oklchCss, toOklch } from "../shared/color";
...
export const contrastProducer: ColorProducer<ContrastOpts> = {
	...
	produce(opts, ctx: ModeCtx) {
		...
		const baseChroma = opts.neutral ? Math.min(seed.c, 0.012) : seed.c * (opts.chroma ?? 1);
		...
		for (let i = 0; i < n; i++) {
			...
			scale[ctx.steps[i]!] = oklchCss(solveStep(target, seed.h, chroma, bgL, contrastOf));
		}
