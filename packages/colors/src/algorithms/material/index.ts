...
import { DEFAULT_TONES } from "./defaults";
import type { ColorScale, MaterialOptions } from "../../types";
 ...
 */
function adjustTonesForContrast(tones: readonly number[], contrastLevel: number): number[] {
 ...
 * @returns ColorScale with 11 steps (50-950)
 */
export function generateScale(options: MaterialOptions): ColorScale {
	const { color, tones = DEFAULT_TONES, chroma: chromaOverride, hue: hueOverride, contrastLevel = 0 } = options;
	...
	const output: Partial<ColorScale> = {};
 ...
 */
export function getContrastTones(backgroundTone: number, ratios: number[]): number[] {
