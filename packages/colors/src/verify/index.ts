 ...
 */
export function nudgeForTarget(
  fg: string,
  bg: string,
  target: number,
  formula: ContrastFormula,
  ...
  const at = (l: number): number =>
    ratioOf(oklchCss(gamutMap({ l, c: f.c, h: f.h })), bg, formula)
  ...
  const best = candidates.reduce((a, b) =>
    Math.abs(b - f.l) < Math.abs(a - f.l) ? b : a,
  )
...
function verifyPairing(
  ...
  return {
    ...
    suggestedFg:
      !meets && suggestFix
        ? (nudgeForTarget(p.fg, p.bg, target, formula) ?? undefined)
        : undefined,
  }
}
