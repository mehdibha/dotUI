...
function detectRootStylesheet({ cwd, cssFile, source, logLevel }) {
	...
	if (envCssFile && !cssFile) {
		const abs = path.isAbsolute(envCssFile) ? envCssFile : path.resolve(cwd, envCssFile);
		if (fileExists(abs)) return abs;
	}

	const envSourceRaw = process.env.TW_AUTOCONTRAST_SOURCE || process.env.TAILWIND_AUTOCONTRAST_SOURCE;
	const allSources = [...toArray(source), ...(envSourceRaw ? envSourceRaw.split(",").map((s) => s.trim()) : [])].filter(
		Boolean,
	);
	...
	for (const entry of allSources) {
		...
		const absFile = path.isAbsolute(normalized) ? normalized : path.resolve(cwd, normalized);
