...
import { createStyleSchema, style, updateStyleConfigSchema, user } from "@dotui/db/schemas";
...
export const styleRouter = {
	...
	setActive: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const isPublicOrUnlisted = ["public", "unlisted"].includes(styleRecord.visibility);
	...
	getPublicStyles: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const styles = await ctx.db.query.style.findMany({
				...
				orderBy: (s, { desc, asc }) => [sortBy === "oldest" ? asc(s.createdAt) : desc(s.createdAt)],
	...
	getMyStyles: protectedProcedure.input(paginationSchema).query(async ({ ctx, input }) => {
		const styles = await ctx.db.query.style.findMany({
	...
	getById: publicProcedure.input(z.object({ id: uuidSchema })).query(async ({ ctx, input }) => {
		const styleRecord = await ctx.db.query.style.findFirst({
	...
	getBySlug: publicProcedure.input(z.object({ slug: z.string().min(1) })).query(async ({ ctx, input }) => {
		...
		if (parts.length === 2) {
			...
			styleRecord = await ctx.db.query.style.findFirst({
				where: and(eq(style.userId, targetUser.id), eq(style.name, styleName)),
				with: {
					user: {
						columns: {
							username: true,
			...
			throw new TRPCError({
				code: "BAD_REQUEST",
				message: "Invalid slug format. Use 'username/stylename' or 'stylename' for public styles",
			});
		}
	...
	create: protectedProcedure.input(createStyleSchema).mutation(async ({ ctx, input }) => {
		...
		const existing = await ctx.db.query.style.findFirst({
			where: and(eq(style.userId, ctx.session.user.id), eq(style.name, input.name)),
		});
	...
	update: protectedProcedure.input(updateStyleConfigSchema).mutation(async ({ ctx, input }) => {
	...
	delete: protectedProcedure.input(z.object({ id: uuidSchema })).mutation(async ({ ctx, input }) => {
		return await ctx.db.transaction(async (tx) => {
			...
			if (ctx.session.user.activeStyleId === input.id) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: "Cannot delete your active style. Please set a different active style first.",
				});
			}
