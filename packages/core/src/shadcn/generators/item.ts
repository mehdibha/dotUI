...
export async function generateShadcnItem(
	...
	const variant =
		registryItemName in config.variants ? config.variants[registryItemName as keyof typeof config.variants] : undefined;
	...
	let shadcnItem = updateRegistryDependencies(registryItem as unknown as RegistryItem, options);
