...
export function generateRegistryBase(options: {
	...
	const registryItem = updateRegistryDependencies(structuredClone(registryBase[0]) as unknown as RegistryItem, options);

	const iconLibrary = iconLibraries.find((lib) => lib.name === options.config.icons.library);

	if (!iconLibrary) {
		throw new Error(`Icon library ${options.config.icons.library} not found`);
	}

	registryItem.dependencies = [...(registryItem.dependencies ?? []), iconLibrary.package];
