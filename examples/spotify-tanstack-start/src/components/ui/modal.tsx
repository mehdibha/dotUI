...
const modalVariants = tv({
  slots: {
    ...
    backdrop:
      "absolute inset-0 bg-overlay/40 backdrop-blur-sm transition-opacity duration-enter group-exiting/modal:duration-exit motion-reduce:transition-none group-entering/modal:opacity-0 group-exiting/modal:opacity-0",
    viewport:
      "@container-size sticky top-0 left-0 flex h-(--visual-viewport-height) w-full justify-center items-center",
    modal:
      "relative flex w-full max-w-[calc(100vw-2rem)] flex-col rounded-lg border border-(--overlay-border) bg-popover shadow-(--shadow-modal,var(--shadow-lg)) [--surface-radius:var(--radius-lg)] sm:max-w-md max-h-[calc(var(--visual-viewport-height)-2rem)] sm:max-h-[calc(var(--visual-viewport-height)*0.9)] transition-[opacity,translate] duration-enter ease-enter exiting:duration-exit exiting:ease-out motion-reduce:transition-none entering:translate-y-2 entering:opacity-0 exiting:translate-y-2 exiting:opacity-0",
  },
});
