...
const tagGroupVariants = tv({
  slots: {
    ...
    tag: "group/tag relative inline-flex w-fit shrink-0 cursor-default items-center justify-center gap-1 rounded-md font-medium whitespace-nowrap outline-hidden transition-colors select-ui data-react-aria-pressable:cursor-interactive **:[svg]:pointer-events-none **:[svg]:shrink-0 focus-visible:focus-ring data-href:cursor-interactive data-selection-mode:disabled:cursor-disabled text-xs/relaxed **:[svg]:not-with-[size]:size-3 has-[button[slot=remove]]:pr-0 **:[button[slot=remove]]:-ml-1 **:[button[slot=remove]]:size-5 **:[button[slot=remove]]:rounded-none **:[button[slot=remove]]:bg-transparent group-data-[size=sm]/tag-group:h-4.25 h-5.25 px-1.5 group-data-[size=lg]/tag-group:h-6.25 group-data-[size=lg]/tag-group:text-sm bg-neutral text-fg-on-neutral disabled:bg-(--disabled-bg,var(--color-neutral)) disabled:text-(--disabled-fg,var(--color-fg-on-neutral)) selected:bg-accent-muted selected:text-fg-accent **:[button[slot=remove]]:text-fg-muted **:[button[slot=remove]]:hover:text-fg",
  },
});
