...
const modalVariants = tv({
  slots: {
    ...
    backdrop: [
      "absolute inset-0 bg-overlay/(--modal-backdrop-opacity) backdrop-blur-sm duration-200 group-exiting/modal:duration-150",
    ...
    modal: [
      "relative flex max-h-[calc(var(--visual-viewport-height)-2rem)] w-full max-w-[calc(100vw-2rem)] flex-col rounded-xl border bg-(--modal-background) shadow-[var(--shadow-overlay,var(--shadow-lg))] [--surface-radius:var(--modal-radius)] sm:max-h-[calc(var(--visual-viewport-height)*.9)]",
