						...
						<SearchCommand items={items} keyboardShortcut>
							<SidebarItem className="mb-2">
								<SidebarTooltip content="Search">
									<Button aria-label="Search" variant="default" size="sm" className="text-fg-muted">
										<SearchIcon />
						...
						{items.map((item) => {
							return (
								<SidebarSection key={item.$id}>
									<SidebarSectionHeading className="text-fg">{item.name}</SidebarSectionHeading>
									{item.type === "folder" && (
										<SidebarList className="gap-0 pl-2">
											{item.children.map((child) => {
												const pageItem = child as DocsPageItem;
												return (
													child.type === "page" && (
			...
			<SidebarFooter>
				...
				>
					...
					<div className="flex flex-col items-center gap-1 group-data-expanded:flex-row">
						<motion.div layout transition={transition}>
							<ThemeToggle size="sm" variant="quiet" />
						</motion.div>
						<AnimatePresence>
							{isMounted && !isPending && session?.user && (
								<motion.div layout transition={transition} className="flex">
									<UserProfileMenu placement="right" size="sm" className="*:data-[slot=avatar]:size-6" />
								</motion.div>
							)}
						</AnimatePresence>
					</div>
