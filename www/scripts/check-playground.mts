...
import { buildSourceOverlay, type ControlSelection } from "../src/modules/docs/codegen/source-overlay.ts";
...
if (!name) {
	console.error("usage: tsx scripts/check-playground.mts <component> [fileSlug]");
	process.exit(2);
}
...
const controls = await buildControlsFromReference(name, controlInput, demoSource);
...
const template = await buildSourceOverlay({ source: demoSource, controls: selections, componentName: name });
const values = Object.fromEntries(
	controls.map((c) => [c.name, c.type === "icon" ? null : (c as { defaultValue?: unknown }).defaultValue]),
);

console.log(`✓ ${name}: overlay OK (${controls.length} controls, ${template.holes.length} holes)`);
