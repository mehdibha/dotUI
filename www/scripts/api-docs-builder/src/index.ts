...
const OUTPUT_DIR = path.join(process.cwd(), "modules/docs/api-reference/generated");
...
async function run(options: RunOptions) {
	...
	for (const file of targetFiles) {
		...
		try {
			...
			if (ast.exports.length > 0) {
				console.log(`  Found ${ast.exports.length} exports: ${ast.exports.map((e) => e.name).join(", ")}`);
			}
	...
	for (const exportNode of propsExports) {
		try {
			...
			console.error(`  ⛔ Error formatting ${exportNode.name}: ${(error as Error).message}`);
		}
	}
...
async function getFilesToProcess(options: RunOptions, configDir: string): Promise<string[]> {
	...
	.command<RunOptions>(
		...
		(command) => {
			return command.option("files", {
				...
				type: "array",
				demandOption: false,
				description: "Glob patterns for files to process. If not provided, all types.ts files are used.",
			});
		},
		run,
	)
