...
export function formatDetailedType(
  ...
  if (type instanceof tae.ExternalTypeNode) {
    ...
    const exportNode = allExports.find(
      (node) => node.name === type.typeName.name,
    );
    if (exportNode) {
  ...
  if (type instanceof tae.UnionNode) {
    const memberTypes = type.types.map((t) =>
      formatDetailedType(t, allExports, visited),
    );
...
export function formatType(
  ...
  if (type instanceof tae.ExternalTypeNode) {
    ...
    if (
      type.typeName.namespaces?.length === 1 &&
      type.typeName.namespaces[0] === "React"
    ) {
  ...
  if (type instanceof tae.IntrinsicNode) {
    return type.typeName
      ? getFullyQualifiedName(type.typeName)
      : type.intrinsic;
  }
  ...
  if (type instanceof tae.ObjectNode) {
    ...
    return `{ ${type.properties
      .map(
        (m) =>
          `${m.name}${m.optional ? "?" : ""}: ${formatType(m.type, m.optional)}`,
      )
      .join(", ")} }`;
  }
  ...
  if (type instanceof tae.TupleNode) {
    ...
    return `[${type.types
      .map((member: tae.AnyType) => formatType(member, false))
      .join(", ")}]`;
  }
...
function createNameWithTypeArguments(typeName: tae.TypeName) {
  if (
    ...
    return `${typeName.name}<${typeName.typeArguments
      .map((ta) => formatType(ta.type, false))
      .join(", ")}>`;
  }
