 ...
 */
function detectHTMLElementExtension(typeName: string, context: ParserContext): string | null {
				...
				if (node.heritageClauses) {
					for (const clause of node.heritageClauses) {
						...
						for (const typeExpr of clause.types) {
							...
							const htmlAttrsMatch = typeText.match(/(?:React\.)?HTMLAttributes<HTML(\w*)Element>/);
							if (htmlAttrsMatch) {
							...
							const htmlPropsMatch = typeText.match(/HTMLProps<HTML(\w+)Element>/);
							if (htmlPropsMatch) {
	...
	function traverse(node: TType): void {
		...
		switch (node.type) {
			...
			case "intersection":
				...
				break;
			case "function":
				(node as { parameters: TType[]; return: TType }).parameters.forEach(traverse);
 ...
 */
function resolveTypeByName(typeName: string, context: ParserContext, _astContext: ConversionContext): TType | null {
...
export async function formatComponentData(exportNode: tae.ExportNode, context: ParserContext) {
	...
	const props = await getPropsWithTypeChecker(exportNode.name, context, astContext, extendsElement);
...
async function getPropsWithTypeChecker(
	...
	const declarationOrder = getPropertyDeclarationOrder(typeName, program, checker);
				...
				for (const prop of properties) {
					...
					if (propType.aliasSymbol) {
						...
						if (typeArgs && typeArgs.length > 0) {
							const argsStr = typeArgs.map((t) => checker.typeToString(t)).join(", ");
							typeString += `<${argsStr}>`;
						}
					} else {
					...
					const defaultValue = defaultTag ? ts.displayPartsToString(defaultTag.text) : undefined;
					...
					if (typeAst) {
						...
						for (const link of linkedTypes) {
							...
							const resolvedType = resolveTypeByName(link.name, context, astContext);
							if (resolvedType) {
					...
					if (currentProp) {
						(Object.keys(currentProp) as Array<keyof FormattedProp>).forEach((key) => {
 ...
 */
function extractRenderProps(propsTypeName: string, context: ParserContext): Record<string, RenderPropInfo> {
	const { program, checker } = context;
...
export function isPublicPropsType(exportNode: tae.ExportNode): boolean {
	...
	if (exportNode.documentation?.hasTag("ignore") || exportNode.documentation?.hasTag("internal")) {
		return false;
	}
...
function extractComponentGroup(componentExportName: string): string {
	...
	const directMatch = componentGroupNames.find((name) => baseName.startsWith(name));
...
function rewriteTypeValue(value: string, componentGroup: string): string {
	...
	if (componentGroup) {
		...
		result = result.replace(new RegExp(`\\b${escaped}(Props|State)\\b`, "g"), `${componentGroup}.$1`);
	}

	return result;
}

function rewriteTypeStringsDeep(node: unknown, componentGroup: string): unknown {
	if (node == null) {
		return node;
	}
