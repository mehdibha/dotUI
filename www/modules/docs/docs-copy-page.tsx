...
function getPromptUrl(baseURL: string, url: string) {
	return `${baseURL}?q=${encodeURIComponent(
		`I’m looking at this dotUI documentation: ${url}.
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
	...
	return (
		<Group>
			<Button size="sm" onPress={() => copyToClipboard(page)}>
			...
			<Menu>
				...
				<Popover placement="bottom end">
					<MenuContent>
						{options.map((option) => (
							<MenuItem key={option.name} href={option.href(url) as Route} target="_blank" rel="noopener noreferrer">
								{option.icon}
								{option.name}
							</MenuItem>
						))}
