...
import { type GroupedPropsData, type PropData, PropsTable } from "./props-table";
...
async function loadApiReference(name: string): Promise<ComponentApiReference | null> {
	try {
 ...
 */
async function transformProp(propName: string, prop: PropDefinition): Promise<PropData> {
	...
	return {
		...
		defaultHighlighted: prop.default ? await highlightCode(prop.default, "ts") : undefined,
 ...
 */
async function transformProps(props: Record<string, PropDefinition>): Promise<PropData[]> {
	return Promise.all(Object.entries(props).map(([name, prop]) => transformProp(name, prop)));
}
...
export async function Reference({ name, className }: ReferenceProps) {
	...
	const groupedData: GroupedPropsData = {
		...
		groups: Object.fromEntries(
			await Promise.all(
				Object.entries(groups).map(async ([groupName, groupProps]) => [groupName, await transformProps(groupProps)]),
			),
		),
	};
	...
	return (
		<div className={cn("flex flex-col gap-4", className)}>
			{data.description && <p className="text-fg-muted">{renderDescription(data.description)}</p>}
			{data.extendsElement && (
