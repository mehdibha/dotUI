...
const GRID_LAYOUT = cn("grid grid-cols-[minmax(120px,1fr)_1fr_2.5rem]", "md:grid-cols-[5fr_7fr_4.5fr_2.5rem]");
			...
			<div className="w-full overflow-hidden rounded-md border text-sm">
				<table className="w-full border-collapse [&>tbody:last-child>tr:last-child>td]:border-b-0 [&>tbody:last-child>tr:last-child]:border-b-0">
					...
					{Object.entries(data.groups).map(([groupName, props]) => (
						<DisclosureGroup key={groupName} title={groupName} defaultExpanded={defaultExpandedGroups.has(groupName)}>
							{props.map((prop) => (
								<PropRows key={prop.name} prop={prop} componentName={componentName} />
							))}
						</DisclosureGroup>
				...
				<td className="overflow-hidden px-3 py-2.5">
					<button type="button" id={id} className="flex items-baseline gap-1 text-left" aria-expanded={isOpen}>
						<code className="whitespace-nowrap font-mono text-[0.8125rem] text-fg">
							{prop.name}
							{prop.required && <sup className="top-[-0.3em] ml-0.5 text-danger">*</sup>}
						</code>
					</button>
				</td>
						...
						<dl className={PANEL_GRID_LAYOUT}>
							...
							<DescriptionItem label="Name">
								<a href={`#${id}`} className="hover:underline">
									<code className="font-mono text-[0.8125rem] text-primary">{prop.name}</code>
								</a>
							</DescriptionItem>
							...
							<DescriptionItem label="Type" hasSeparator>
								{prop.typeAst ? (
									...
									<TypeDisplay type={prop.type} highlighted={prop.typeHighlighted} />
								)}
							</DescriptionItem>
							...
							{prop.default !== undefined && (
								<DescriptionItem label="Default" hasSeparator>
									<code className="font-mono text-[0.8125rem]">{prop.defaultHighlighted ?? prop.default}</code>
								</DescriptionItem>
							)}
						</dl>
					</td>
...
function TypeDisplay({ type, highlighted }: TypeDisplayProps) {
	...
	return <code className="font-mono text-[0.8125rem]">{highlighted ?? type}</code>;
}
