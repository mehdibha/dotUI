...
export const DemoCodeBlock = ({
  ...
  const { hasControls, dynamicPreview, dynamicSource, isExpanded } =
    useDemoContext();

  const previewContent =
    hasControls && preview ? (
      <DynamicCodeBlock
        code={(dynamicPreview ?? preview).trim()}
        lang="tsx"
  ...
  const codeContent =
    hasControls && source ? (
      <DynamicCodeBlock
        code={(dynamicSource ?? source).trim()}
        lang="tsx"
...
const ExpandButton = () => {
  const { isExpanded, setExpanded } = useDemoContext();
  return (
    <Button
      variant="quiet"
      size="sm"
