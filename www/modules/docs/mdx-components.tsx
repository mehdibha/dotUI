...
import { Reference, type ReferenceProps } from "./api-reference/reference";
import { CodeBlock, Pre } from "./code-block";
import { CodeBlockTab, CodeBlockTabs, CodeBlockTabsList, CodeBlockTabsTrigger } from "./code-block/code-block-tabs";
...
export const mdxComponents: MDXComponents = {
	h1: (props: HeadingProps) => <Heading level={1} className="mt-2 scroll-m-20 font-bold text-4xl" {...props} />,
	...
	h3: (props: HeadingProps) => (
		<Heading level={3} className="mt-8 scroll-m-20 font-semibold text-xl tracking-tight" {...props} />
	),
	h4: (props: HeadingProps) => (
		<Heading level={4} className="mt-8 scroll-m-20 font-semibold text-lg tracking-tight" {...props} />
	),
	h5: (props: HeadingProps) => (
		<Heading level={5} className="mt-8 scroll-m-20 font-semibold text-lg tracking-tight" {...props} />
	),
	h6: (props: HeadingProps) => (
		<Heading level={6} className="mt-8 scroll-m-20 font-semibold text-base tracking-tight" {...props} />
	),
	PageTabs,
	PageTabPanel,
	...
	CodeBlockTab: ({ value, ...props }: { value: string }) => <CodeBlockTab id={value} {...props} />,
	...
	tr: ({ className, ...props }) => <tr className={cn("m-0 border-t p-0 text-sm", className)} {...props} />,
	th: ({ className, ...props }) => (
		<th
			className={cn(
	...
	Steps: ({ ...props }: React.ComponentProps<"div">) => (
		<div className="[&>h3]:step ml-4 border-l pl-8 [counter-reset:step]" {...props} />
	),
	Reference: ({ name }: ReferenceProps) => <Reference name={name} className="mt-4" />,
};

function Link(props: Omit<LinkProps, "children"> & { children: React.ReactNode }) {
	return (
		<NavLink target={props.href?.startsWith("/") ? "_self" : "_blank"} {...props} className="inline">
			{props.children}
