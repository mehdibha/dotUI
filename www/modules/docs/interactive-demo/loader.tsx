...
import type {
  ...
  PropDefinition,
} from "@/modules/docs/api-reference/types";
...
export async function loadComponent(name: string) {
  ...
  if (!component) {
    throw new Error(
      `Component ${componentExportName} not found in ${name}/demos/playground`,
    );
  }
...
export async function buildControlsFromReference(
  ...
  const referenceExport = await import(
    `@/modules/docs/api-reference/generated/${name}.json`
  );

  const reference = referenceExport.default as
    | ComponentApiReference
    | undefined;
  ...
  for (const input of controlInputs) {
    ...
    if (!prop) {
      ...
      throw new Error(
        `Prop ${propName} not found in API reference for ${name}`,
      );
    }
 ...
 */
export async function enrichControlsWithReference(
  name: string,
  controls: Control[],
): Promise<Control[]> {
  const referenceExport = await import(
