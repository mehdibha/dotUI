		...
		<div className="flex items-center justify-between px-2">
			...
			<div className="flex items-center gap-2">
				<ThemeModeSwitch
					...
					onChange={(isSelected) => setActiveMode(isSelected ? "light" : "dark")}
				/>
				<Button className="font-mono max-lg:hidden [&_svg]:size-4 [&_svg]:text-fg-muted" onPress={handleCopy} size="sm">
					{isCopied ? <CheckIcon className="fade-in animate-in" /> : <TerminalIcon className="fade-in animate-in" />}
					<span className="truncate text-xs">npx shadcn@latest add @dotui/{name}</span>
				</Button>
				{activeStyle ? (
					<LinkButton
...
const BlockViewView = ({ block }: { block: RegistryItem }) => {
	...
	return (
		<div className={cn("overflow-hidden rounded-lg border bg-muted", isLoading && "animate-pulse")}>
			<iframe
				src={activeStyle ? `/view/${activeStyle?.user.username}/${activeStyle?.name}/${block.name}` : undefined}
