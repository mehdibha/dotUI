...
import { ColorPickerContent, ColorPickerTrigger } from "@dotui/registry/ui/color-picker";
	...
	return (
		<div>
			...
			<div className="mt-2 flex items-center gap-2">
				<form.AppField name={`theme.colors.modes.${resolvedMode}.scales.${scaleId}.colorKeys`}>
					{(field) => {
						return (
...
function RatiosEditor({ scaleId }: { scaleId: ScaleId }) {
	...
	useStore(form.store, (state) => state.values.theme.colors.modes[resolvedMode].scales[scaleId].ratios);
	...
	return (
		<div className="flex-1">
			...
			<div className="flex items-stretch gap-4">
				...
				<form.AppField mode="array" name={`theme.colors.modes.${resolvedMode}.scales.${scaleId}.ratios`}>
					{(field) => {
						return (
											...
											<TableRow key={i}>
												...
												<TableCell>
													{requiredContrast && (
														<form.Subscribe
															selector={(state) =>
																state.values.theme.colors.modes[resolvedMode].scales[scaleId].ratios[i]
															}
														>
															{(ratio) =>
																ratio && (
																	<Badge
																		variant={ratio > requiredContrast ? "success" : "danger"}
...
const useDynamicGradient = (scaleId: ScaleId) => {
	...
	const [neutralColorKeys, colorKeys, lightness, saturation, contrast] = useStore(form.store, (state) => [
