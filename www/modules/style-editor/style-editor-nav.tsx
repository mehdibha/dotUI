...
export function StyleEditorNav({ children, className, ...props }: { children: React.ReactNode } & TabsProps) {
	const pathname = usePathname();
	const { username, style: styleName } = useStyleEditorParams();

	const menuItems = React.useMemo(() => getMenuItems(username, styleName), [username, styleName]);
	...
	return (
		<>
			<div className="mt-4 @lg:hidden">
				<Menu>
					<Button variant="quiet" className="flex w-full rounded-none border-y text-left sm:px-6 lg:px-10">
						{selectedTab?.icon}
			...
			<Tabs selectedKey={pathname} className={cn("@max-lg:hidden", className)} {...props}>
