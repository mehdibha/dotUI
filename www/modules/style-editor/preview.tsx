...
const [PreviewProvider, usePreviewContext] = createScopedContext<PreviewContextType>("PreviewRoot");
...
export const PreviewRoot = ({ children }: { children: React.ReactNode }) => {
	...
	const { containerRef, width, setWidth, handleMouseDown, isDragging } = useHorizontalResize({
		minWidth: 320,
		maxWidth: 768,
		initialWidth: 768,
		edge: "left",
	...
	return (
		...
		>
			...
			>
				...
				>
					<div className={cn("flex size-full flex-col overflow-hidden rounded-md border bg-bg")}>{children}</div>
				</motion.div>
			</motion.div>
		</PreviewProvider>
	);
		...
		<div className="flex items-center justify-between gap-2 border-t-inherit border-b bg-card p-1">
			<div className="flex items-center gap-1">
				...
				<Select value={block} onChange={(key) => setBlock(key as string)}>
					...
					<SelectContent items={registryBlocks.filter((block) => block?.categories?.includes("featured"))}>
