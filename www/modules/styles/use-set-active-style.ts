...
export function useSetActiveStyle() {
	...
	return useMutation({
		...
		onMutate: async (variables) => {
			...
			const previousStyle = queryClient.getQueryData(trpc.style.getActive.queryKey());
			queryClient.setQueryData(trpc.style.getActive.queryKey(), variables.styleId);
			return { previousStyle, newStyle: variables.styleId };
		},
		onError: (_err, _variables, context) => {
			if (context?.previousStyle !== undefined) {
				queryClient.setQueryData(trpc.style.getActive.queryKey(), context.previousStyle);
			}
		},
		onSettled: () => {
