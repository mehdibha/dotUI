...
Multi-series wide data is grouped side by side by default. Set `grouped` to group long-format series too. Without grouping, series that land on the same category share the band and stack from zero rather than overlapping — set `y1` (as `stackY` does) when you want to control the interval yourself.
