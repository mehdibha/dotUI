...
`Combobox` is assembled from the field, input, list-box, and popover primitives. `InputGroup` wraps the text `Input` and an `InputGroupAddon` holding the `ComboboxTrigger` button, which carries the caret; the `Popover` holds a `ListBox` of `ListBoxItem` options.
...
<Reference name="combobox-trigger" />
