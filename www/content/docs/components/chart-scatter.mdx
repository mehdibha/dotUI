...
`series` splits rows into colored groups and turns on the legend. Set `seriesOrder` to lead the color slots and the legend — groups the data carries but `seriesOrder` omits follow in the order they first appear — and `labels` maps raw keys to display names.
...
- `ariaLabel` is required and names the figure; `ariaDescription` adds the longer explanation a sighted reader gets from the surrounding copy.
...
- Color alone never carries the distinction — position carries the relationship, and the legend and tooltip name every group. When a group's name matters more than its hue, say so in `labels`.
