...
```package-install
npx shadcn@latest add https://dotui.org/r/{{STYLE_NAME}}/drop-zone.json
```
...
<ComponentPreview
  name="drop-zone/demos/file-trigger"
  preview={`<DropZone>
    <UploadIcon />
    <DropZoneLabel>Drag and drop files here</DropZoneLabel>
    <FileTrigger>
...
<ComponentPreview
  name="drop-zone/demos/visual-feedback"
  preview={`<DropZone
