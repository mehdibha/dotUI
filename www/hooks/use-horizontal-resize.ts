...
export const useHorizontalResize = (options?: ResizeOptions) => {
	const { minWidth: minWidthProp = 300, initialWidth, edge = "right", onResize } = options || {};
	...
	const handleMouseDown = React.useCallback(
		(e: React.MouseEvent) => {
			...
			const handleMouseMove = (e: MouseEvent) => {
				...
				const newWidth = Math.min(Math.max(minWidth.current, startWidth.current + delta), maxWidth.current);
				setWidth(newWidth);
				onResize?.({
					newWidth,
					delta,
