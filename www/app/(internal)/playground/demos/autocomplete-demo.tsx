...
import { Autocomplete, SelectableCollectionContext, useFilter } from "react-aria-components";
			...
			<Dialog>
				...
				>
					<DialogContent className="w-full max-w-2xl gap-0 rounded-4xl bg-neutral p-0!">
						...
						>
							<InputGroup size="lg" className="w-full border-0 p-1.5 [--radius-factor:9999] [--spacing:0.4em]">
								<InputAddon>
								...
								<TextField autoFocus aria-label="Search" className="w-full">
									<Input placeholder="What do you want to know?" className="min-h-0 w-full py-2" />
								</TextField>
								<InputAddon>
									<SelectableCollectionContext value={null}>
										<Select aria-label="Model" className="w-fit" defaultValue="grok-4-fast">
							...
							>
								{(item) => <ListBoxItem id={item.id}>{item.question}</ListBoxItem>}
							</ListBox>
						</Autocomplete>
					</DialogContent>
				</Overlay>
