...
import { Description, FieldGroup, Fieldset, Label } from "@dotui/registry/ui/field";
		...
		<div className="grid grid-cols-2 gap-8">
			<Fieldset className="[&_[data-slot=input]]:w-full [&_[data-slot=text-field]]:w-full [&_[data-slot=textarea]]:w-full">
				<FieldGroup>
					<TextField>
						<Label>Name</Label>
						<Description>Enter your name so it is long enough to test the layout.</Description>
						<Input />
					</TextField>
					<TextField>
						<Label>Mesage</Label>
						<Description>Enter your message so it is long enough to test the layout.</Description>
						<TextArea />
					</TextField>
					<TextField>
						<Label>Mesage</Label>
					...
					<Checkbox className="flex-row-reverse justify-between">I agree to the terms and conditions</Checkbox>
					<Switch>Dark mode</Switch>
					<Switch className="flex-row-reverse justify-between">Dark mode</Switch>
					<Separator />
					<Checkbox>
						Enable Touch ID and Face ID to make it even faster to unlock your device. This is a long label to test the
						layout.
					</Checkbox>
					<Description className="ml-4">Enable Touch ID to quickly unlock your device.</Description>
