		...
		items: [
			{
				...
				description: "The dialog is rendered as a modal in desktop and as a drawer on mobile.",
...
const Section = ({
	...
	const [controlStates, setControlStates] = React.useState<Record<string, boolean>>(() =>
		uniqueProps.reduce(
			(acc, prop) => {
				acc[prop] = false;
				return acc;
	...
	return (
		<div className="flex flex-col gap-2 space-y-2">
			<div className="flex items-center justify-between gap-2">
				<h3 id={id} className="mt-8 scroll-m-20 font-heading font-semibold text-xl tracking-tight first:mt-0">
					{title}
				</h3>
			</div>
			...
			<div className={cn("relative rounded-lg border bg-bg p-6 pt-12", className)}>
				<div className="absolute top-4 right-4 flex gap-8 text-fg-muted text-sm">
					...
					<Switch size="sm" isSelected={isLoading} onChange={setIsLoading} className="flex-row-reverse gap-1">
						<SwitchIndicator>
							<SwitchThumb />
						</SwitchIndicator>
