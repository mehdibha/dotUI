	...
	return (
		<div
			className={cn(
				...
				hasToc && "grid grid-cols-1 gap-10 xl:max-w-5xl xl:grid-cols-[minmax(0,1fr)_minmax(180px,220px)]",
			)}
		>
			<div className="pt-4 pb-24 md:pt-10 lg:pt-16">
				<h1 className="font-semibold text-2xl lg:text-3xl">{page.data.title}</h1>
				...
				<div className="mt-20 space-y-4">{lastModified && <PageLastUpdate date={lastModified} />}</div>
			</div>
			<TOCProvider toc={toc}>
				{hasToc && (
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
	...
	return {
		...
		openGraph: {
			title: page.data.title,
			description: page.data.description ? truncateOnWord(page.data.description, 148, true) : undefined,
			type: "article",
			url: page.url,
