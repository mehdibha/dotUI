...
const REGISTRY_DIST_PATH = path.resolve(process.cwd(), "../packages/registry-builder/dist");
 ...
 */
export async function loadItemJson(name: string, category: string = "ui"): Promise<ItemJson | null> {
	try {
 ...
 */
export async function loadItemFromAnyCategory(name: string): Promise<ItemJson | null> {
 ...
 */
export async function loadSpecialItem(name: "theme" | "base" | "all" | "registry"): Promise<unknown> {
	try {
		switch (name) {
			case "registry":
				return loadManifest();
