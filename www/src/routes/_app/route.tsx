  ...
  return (
    <DrawerProvider>
      ...
      {/* The mobile menu (a left drawer in the header) pushes the page aside
          ...
          is non-zero), and rounds its corner over the dark layer above.
          Page drawers (docs demos, studio) live under their own provider
          below so they never trigger the push. */}
      <DrawerIndent className="origin-left duration-[calc(500ms*var(--indent-transition))] [--header-height:--spacing(14)] [--indent-transition:calc(1-clamp(0,calc(var(--drawer-swipe-progress,0)*100000),1))] data-active:transform-[translate3d(calc(var(--mobile-menu-width)*(1-var(--drawer-swipe-progress,0))),0,0)] data-active:rounded-3xl">
        <Header items={items} />
        <DrawerProvider>
          <main id="content">
            <Outlet />
