...
const shimmerKeyframes: Keyframe[] = [{ backgroundPosition: "100%" }, { backgroundPosition: "0%" }];
const pulseKeyframes: Keyframe[] = [{ opacity: 1 }, { opacity: 0.5 }, { opacity: 1 }];
...
function useSyncedSkeletonAnimation(
	...
	useIsomorphicLayoutEffect(() => {
		...
		const stopAnimations = (element: HTMLElement) => {
			...
			for (const skeletonElement of element.querySelectorAll<HTMLElement>(skeletonSelector)) {
...
export function Skeleton({ className, children, isLoading, ...props }: SkeletonProps) {
