...
const { useStyles, styles } = createStyles(cardMeta, {
  base: {
    slots: {
      root: "group/card flex flex-col rounded-(--card-radius) border border-(--card-border) bg-card shadow-[var(--shadow-card,0_0_#0000)] [--surface-radius:var(--card-radius)] has-[>img:first-child]:pt-0 *:[img]:first:rounded-t-(--card-radius) *:[img]:last:rounded-b-(--card-radius)",
      header:
