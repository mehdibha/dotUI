...
const { useStyles, styles } = createStyles(listMeta, {
  base: {
    slots: {
      ...
      item: [
        "relative grid w-full grid-cols-[auto_minmax(0,1fr)_auto_auto] content-center items-center px-(--list-px) text-fg outline-hidden select-ui [--list-separator:var(--list-px)] has-data-[slot=list-item-icon]:[--list-separator:calc(var(--list-px)+var(--list-icon)+--spacing(3))] *:[[role=gridcell]]:contents",
