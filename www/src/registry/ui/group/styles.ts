...
const { useStyles, styles } = createStyles(groupMeta, {
	base: {
		slots: {
			...
			text: "flex items-center gap-2 rounded-md border bg-card px-4 font-medium text-sm shadow-xs **:[svg]:pointer-events-none **:[svg]:not-with-[size]:size-4",
		},
		variants: {
			orientation: {
				horizontal: {
					root: [
						"-space-x-px not-has-data-group:*:not-last:rounded-r-none not-has-data-group:*:not-first:rounded-l-none",
