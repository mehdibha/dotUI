...
const { useStyles, styles } = createStyles(dialogMeta, {
	base: {
		slots: {
			content:
				"relative flex max-h-[inherit] min-h-0 flex-col gap-4 overflow-y-auto in-data-popover:p-4 p-6 outline-none has-data-command:p-0",
			...
			body: "-mx-6 in-data-popover:-mx-4 flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto in-data-popover:px-4 px-6 [@container_(height<31.25rem)]:shrink-0 [@container_(height<31.25rem)]:overflow-visible",
