...
function useContextMenuTrigger({
  ...
  const anchorPositionRef = React.useRef<ContextMenuAnchor>({
    x: 0,
    y: 0,
    size: 0,
    key: 0,
  ...
  const longPressTimeoutRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null)
  const allowMouseUpTimeoutRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null)
  const allowMouseUpRef = React.useRef(false)
  const [anchor, setAnchor] = React.useState<ContextMenuAnchor>({
    x: 0,
    y: 0,
    size: 0,
    key: 0,
  ...
  const anchorRefCallback = React.useCallback(
