...
const tasks: Task[] = [
	{ id: "task-1", task: "Finalize onboarding", status: "In Progress", assignee: "maya", estimate: 6, due: "May 20" },
	{ id: "task-2", task: "Review audit logs", status: "Todo", assignee: "owen", estimate: 3, due: "May 21" },
	{ id: "task-3", task: "Ship billing copy", status: "Backlog", assignee: "nora", estimate: 2, due: "May 24" },
	{ id: "task-4", task: "Tune empty states", status: "Done", assignee: "amir", estimate: 4, due: "May 25" },
	{ id: "task-5", task: "Validate invite flow", status: "Todo", assignee: "maya", estimate: 5, due: "May 27" },
];
				...
				<TableBody>
					{tasks.map((task) => (
						<TableRow key={task.id} textValue={task.task}>
							...
							<TableCell>
								<EditableCellControl label={`Edit estimate for ${task.task}`} value={`${task.estimate}h`}>
									<Input
...
function EditableCellControl({ children, label, value }: { children: ReactNode; label: string; value: ReactNode }) {
	return (
		<div className="flex min-w-0 items-center justify-between gap-2">
			<span className="min-w-0 truncate">{value}</span>
			<Dialog>
				<Button aria-label={label} variant="quiet" size="sm" isIconOnly className="-mr-1">
					<PencilIcon />
				</Button>
