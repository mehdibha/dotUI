...
) as Record<StatusValue, (typeof statuses)[number]>
const priorityByValue = Object.fromEntries(
  priorities.map((priority) => [priority.value, priority]),
) as Record<PriorityValue, (typeof priorities)[number]>
const labelByValue = Object.fromEntries(
  labels.map((label) => [label.value, label]),
) as Record<LabelValue, (typeof labels)[number]>
...
const taskColumns: ColumnDef<Task>[] = [
  ...
  },
  {
    ...
    sortingFn: (a, b, columnId) => {
      return statusByValue[
        a.getValue<StatusValue>(columnId)
      ].label.localeCompare(
  ...
  },
  {
    ...
    filterFn: selectedOptionsFilter,
    sortingFn: (a, b, columnId) => {
      return priorityByValue[
        a.getValue<PriorityValue>(columnId)
      ].label.localeCompare(
...
export default function Demo() {
  ...
  const selectedKeys = React.useMemo(
    () => rowSelectionToSelection(rowSelection),
    [rowSelection],
  )
  ...
  const updatePriorityFilter = React.useCallback(
    (selection: Selection) => {
      priorityColumn?.setFilterValue(
        selectionToFilterValue(selection, priorities),
      )
      table.setPageIndex(0)
    },
  ...
  const updateSelection = React.useCallback(
    (keys: Selection) => {
      setRowSelection((currentSelection) =>
        selectionToRowSelection(keys, currentRows, currentSelection),
      )
    },
    [currentRows],
  )
...
function ViewOptions({
  ...
  columns: TanStackColumn<Task, unknown>[]
}) {
...
function selectionToFilterValue(
  selection: Selection,
  options: readonly FilterOption[],
) {
  const values =
