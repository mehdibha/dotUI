...
const { useStyles, styles } = createStyles(popoverMeta, {
  base: {
    slots: {
      popover: [
        "popover z-50 min-w-[max(var(--trigger-width),--spacing(32))] origin-(--trigger-anchor-point) rounded-(--popover-radius) border bg-popover shadow-[var(--shadow-overlay,var(--shadow-md))] forced-color-adjust-none outline-none [--surface-radius:var(--popover-radius)]",
