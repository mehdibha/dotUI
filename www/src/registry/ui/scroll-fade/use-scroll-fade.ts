...
function useScrollFade({ ref }: UseScrollFadeProps) {
  ...
  React.useLayoutEffect(computeOverflow)
  ...
  React.useLayoutEffect(() => {
    ...
    if (typeof ResizeObserver !== 'undefined') {
      ...
      observeScrollFadeResizeTargets(resizeObserver, element)
    }
  ...
  return { recompute: computeOverflow }
}
...
function updateOverflowState(element: HTMLElement) {
  ...
  const scrollLeftFromStart = hasOverflowX
    ? normalizeScrollOffset(
        getScrollLeftFromStart(element, direction, maxScrollLeft),
        maxScrollLeft,
      )
    : 0
  const scrollTopFromStart = hasOverflowY
    ? normalizeScrollOffset(element.scrollTop, maxScrollTop)
    : 0
  const scrollLeftFromEnd = hasOverflowX
    ? maxScrollLeft - scrollLeftFromStart
    : 0
  ...
  setBooleanAttribute(
    element,
    'data-overflow-x-end',
    hasOverflowX && scrollLeftFromEnd > OVERFLOW_EDGE_THRESHOLD,
  )
  setBooleanAttribute(
    element,
    'data-overflow-y-start',
    hasOverflowY && scrollTopFromStart > OVERFLOW_EDGE_THRESHOLD,
  )
  setBooleanAttribute(
    element,
    'data-overflow-y-end',
    hasOverflowY && scrollTopFromEnd > OVERFLOW_EDGE_THRESHOLD,
  )
...
function observeScrollFadeResizeTargets(
...
function getScrollLeftFromStart(
  element: HTMLElement,
  direction: string,
