...
const { useStyles, styles } = createStyles(tokenFieldMeta, {
  base: {
    slots: {
      ...
      input: [
        "min-h-16 w-full rounded-(--studio-input-radius) border border-border-control bg-field px-2.5 py-2 text-base outline-none sm:text-sm",
        "transition-[box-shadow,border-color,color] duration-(--studio-input-state-duration) ease-(--studio-input-state-ease)",
