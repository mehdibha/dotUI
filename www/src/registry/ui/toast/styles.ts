...
const { useStyles, styles } = createStyles(toastMeta, {
	base: {
		slots: {
			viewport: [
				"focus-reset fixed z-50 mx-auto flex w-[calc(100vw-(var(--toast-inset)*2))] max-w-96 outline-none [--toast-inset:--spacing(4)] sm:[--toast-inset:--spacing(6)]",
				"data-[position*=top]:top-(--toast-inset) data-[position*=bottom]:bottom-(--toast-inset)",
				"data-[position*=right]:right-(--toast-inset) data-[position*=center]:left-1/2 data-[position*=left]:left-(--toast-inset) data-[position*=center]:-translate-x-1/2",
			],
			toast: [
				"focus-reset focus-visible:focus-ring absolute z-[calc(50-var(--toast-index))] h-(--toast-calc-height) w-full select-none overflow-hidden rounded-lg border bg-card text-fg shadow-lg outline-none",
			...
			content:
				"pointer-events-auto flex min-h-12 items-center justify-between gap-1.5 overflow-hidden px-3.5 py-3 text-sm transition-opacity duration-200 data-behind:not-data-expanded:pointer-events-none data-behind:opacity-0 data-expanded:opacity-100",
			...
			title: "font-medium text-sm leading-snug empty:hidden",
			description: "text-fg-muted text-sm leading-snug empty:hidden",
			actions: "ml-2 flex shrink-0 items-center gap-1",
			action:
				"focus-reset focus-visible:focus-ring inline-flex h-7 max-w-32 items-center justify-center rounded-md bg-neutral px-2.5 font-medium text-fg-on-neutral text-xs transition-colors empty:hidden hover:bg-neutral-hover active:bg-neutral-active **:[span]:truncate",
		},
		variants: {
			position: {
				"top-left": {},
