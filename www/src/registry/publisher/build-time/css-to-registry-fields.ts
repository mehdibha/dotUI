...
function addNodeToFields(fields: RegistryCssFields, node: CssNode): void {
	if (node.type === "atrule" && node.name === "theme") {
		addDeclarationsToVars(fields, "theme", node.nodes, { stripCustomPropertyPrefix: false });
		return;
	}
...
function addDeclarationsToVars(
	...
	const declarations = nodes.filter(
		(node): node is DeclarationNode => node.type === "decl" && node.prop.startsWith("--"),
	);
...
function cssVarKey(prop: string, options: { stripCustomPropertyPrefix: boolean }): string {
...
class CssParser {
	...
	private parseNode(): CssNode | undefined {
		...
		if (endChar === "{") {
			...
			return prelude.startsWith("@") ? atRuleWithNodes(prelude, nodes) : { type: "rule", selector: prelude, nodes };
		}
	...
	private readPreludeEnd(): string | undefined {
		...
		for (; this.index < this.input.length; this.index += 1) {
			...
			if (parenDepth === 0 && (char === "{" || char === ";" || char === "}")) return char;
		}
		return undefined;
	}
