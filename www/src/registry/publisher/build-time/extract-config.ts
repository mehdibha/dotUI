...
function unwrap(expr: Expression): Expression {
	if (expr.isKind(SyntaxKind.AsExpression) || expr.isKind(SyntaxKind.TypeAssertionExpression)) {
...
function exprToValue(expr: Expression, filePath: string): unknown {
	...
	if (node.isKind(SyntaxKind.PrefixUnaryExpression)) {
		...
		if (text.startsWith("-") || text.startsWith("+")) {
			...
			if (typeof innerVal === "number") return text.startsWith("-") ? -innerVal : innerVal;
		}
		throw new Error(`[publisher/extract] unsupported unary expression in ${filePath}: ${text}`);
	}

	if (node.isKind(SyntaxKind.ArrayLiteralExpression)) {
		return node.getElements().map((el) => {
			if (el.isKind(SyntaxKind.SpreadElement)) {
				throw new Error(`[publisher/extract] spread elements are not supported (${filePath})`);
			}
	...
	if (node.isKind(SyntaxKind.ObjectLiteralExpression)) {
		...
		for (const property of node.getProperties()) {
			if (property.isKind(SyntaxKind.PropertyAssignment)) {
				...
				if (!initializer) {
					throw new Error(`[publisher/extract] property without initializer in ${filePath}`);
				}
				obj[propertyKey(nameNode, filePath)] = exprToValue(initializer, filePath);
				continue;
			}
...
function propertyKey(nameNode: Node, filePath: string): string {
	...
	throw new Error(`[publisher/extract] unsupported property key ${nameNode.getKindName()} in ${filePath}`);
}
...
function extractFromSourceFile(sourceFile: SourceFile): StylesConfig {
	...
	const target = calls.find((call) => {
		...
		return expr.isKind(SyntaxKind.Identifier) && expr.getText() === "createStyles";
	});

	if (!target) {
		throw new Error(`[publisher/extract] no createStyles(...) call found in ${filePath}`);
	}

	const args = target.getArguments();
	if (args.length < 2) {
		throw new Error(`[publisher/extract] createStyles() missing config argument in ${filePath}`);
	}

	const configArg = args[1];
	if (!configArg || !Node.isExpression(configArg)) {
		throw new Error(`[publisher/extract] createStyles() second argument is not an expression in ${filePath}`);
	}

	const value = exprToValue(configArg, filePath);
	if (typeof value !== "object" || value === null || Array.isArray(value)) {
		throw new Error(`[publisher/extract] createStyles() config must be an object literal in ${filePath}`);
	}
