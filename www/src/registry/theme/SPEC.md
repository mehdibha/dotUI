...
- `base/colors.css` (ramps + the referenced semantic block) is generated by
  `pnpm build:registry` and is site-only; drift is a CI diff, not a parity
  test.
- Export ships **semantic tokens only, as literals** — the ramps never leave
  the site. `semanticLiterals` flattens every token (mixes computed in
  OKLab) per mode into shadcn's own shape: `cssVars.light`/`.dark` hold
  `--bg: oklch(…)`, `cssVars.theme` aliases `--color-bg: var(--bg)` in
...
`neutralTint`, `preserveSeed`, per-token remapping ✅ (the `scales` picker pools as a UI — mode-agnostic
`{palette, job}` rows; per-mode pairs stay config/preset-level), chart
palette controls.

Guarantee policy, high-contrast modes and border-contrast targets were
removed (Sept 2026): out of scope for launch — the engine's own floors are
the only contrast promise.
