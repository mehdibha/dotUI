...
import { fromKernelPaletteName, PALETTE_ORDER, STATUS_PALETTES, toKernelPaletteName } from "./palettes";

describe("palette identity (SSOT)", () => {
	it("orders the six palettes with the neutral backbone + accent first", () => {
		expect(PALETTE_ORDER).toEqual(["neutral", "accent", "success", "warning", "danger", "info"]);
	});

	it("lists the four status palettes (PALETTE_ORDER minus neutral + accent)", () => {
		expect([...STATUS_PALETTES]).toEqual(PALETTE_ORDER.filter((p) => p !== "neutral" && p !== "accent"));
	});
