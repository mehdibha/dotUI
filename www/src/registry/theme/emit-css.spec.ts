...
describe("resolveTarget", () => {
	...
	it("renders a mix as color-mix()", () => {
		expect(
			resolveTarget({ mix: { space: "oklab", stops: [{ ref: "neutral-950" }, 15, { value: "transparent" }] } }),
		).toBe("color-mix(in oklab, var(--neutral-950) 15%, transparent)");
	});

	it("renders nested mixes recursively", () => {
		expect(resolveTarget({ mix: { space: "oklch", stops: [{ onOf: "accent-500" }, 50, { ref: "neutral-50" }] } })).toBe(
			"color-mix(in oklch, var(--on-accent-500) 50%, var(--neutral-50))",
		);
	});
});
...
describe("emitCss per-mode targets", () => {
	it("emits the light value for a per-mode target", () => {
		const vocab: SemanticVocabulary = {
			"color-bg": { target: { light: { ref: "neutral-50" }, dark: { ref: "neutral-950" } }, category: "background" },
		};
	...
	it("falls back to the first mode when no light target is present", () => {
		const vocab: SemanticVocabulary = {
			"color-bg": { target: { dark: { ref: "neutral-900" } }, category: "background" },
		};
		const fields = cssToRegistryFields(emitCss(vocab));
