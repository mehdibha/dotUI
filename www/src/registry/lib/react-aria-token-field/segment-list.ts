/*
 * Vendored from adobe/react-spectrum#10318 (react-stately TokenSegmentList),
...
export class TokenSegmentList<T = any> {
  ...
  delete(
    ...
    let boundary = this.findBoundaryWithSegmenter(
      position,
      segmenter,
      direction,
    )
  ...
  if (typeof search === 'string') {
    if (direction === Direction.Backward) {
      return text.lastIndexOf(
        search,
        fromOffset !== undefined ? fromOffset - 1 : text.length - 1,
      )
    }
