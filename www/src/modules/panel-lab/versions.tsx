...
export const PANEL_VERSIONS: PanelVersion[] = [
  ...
  },
  {
    ...
    label: "v2 (wip)",
    summary:
      "Color modes become a user-defined set — one to many, with archetypes and per-mode contrast. Shape and Space split; radius speaks px (#575) with a nested-corner preview and a corner-shape axis. Space grows into the spatial system — unit × density × control size, resolved over a live form specimen. Surfaces lands as its own chapter (#590): a delineation recipe (hairline · adaptive · shadow · outline) that absorbs shadows and the overlay material. Every section opens on a hero following one contract (hero.tsx): shared stage, hover-peek/click-pin inspection, mono readout — Type gains a live role specimen plus the axes shipped systems actually expose (a hand-tuned ladder with a heading size adjust, heading weight, heading tracking) and an Auto heading that follows the body (#563/#565), Icons a real-registry grid. Components splits into per-family sections — Buttons and Inputs first — each opening on working specimens. Buttons owns the full axis model: style is a family look (flat · outline · raised · elevated) reshaping every fill variant — the variant enum stays API — plus hover · press effects, group layout, and an Advanced disclosure (selected segment, separator, transition). Focus is a dedicated chapter (six-system audit): a shared Color both categories draw from, then Controls and Inputs blocks, each opening on a focused specimen that heads its own ControlGroup. Rows appear only when the chosen style reads them — gap size under a gap offset, halo spread and strength, focused border width — so no knob sits dead. Menu items highlight instead of ringing.",
    chapters: [
      {
        id: "color",
        label: "Color",
