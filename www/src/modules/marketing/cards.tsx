  ...
  return (
    <div className="flex flex-col [--grid-max:1500px] [--rail-gap:--spacing(4)] [--rail-peek:2.5rem] sm:[--rail-peek:3.5rem] md:[--rail-peek:5rem] lg:[--rail-peek:7rem]">
      {/* Flex row: [left rail | gap | capped real grid | gap | right rail]. The rails
			    ...
			    The fade has to swallow the ragged column bottoms: the rail + 2-column main
			    packs tall, uneven columns at every width, so the fade is deep enough to
			    cover their spread throughout. `--mask-solid` is where content is still
			    fully opaque; `--mask-clear` where it's fully gone.
      ...
      <div className="relative flex justify-center gap-4 overflow-hidden [mask-image:linear-gradient(to_bottom,black_calc(100%_-_var(--mask-solid)),transparent_calc(100%_-_var(--mask-clear)))] [--mask-clear:420px] [--mask-solid:980px]">
        <SkeletonRail side="left" />
        {/* The centered, capped real grid (shared with the /create preview), sized as
				    a flex item that bleeds past the rails on small screens and caps at
				    `--grid-max` on large ones (rail + 2-column main). Kept interactive on purpose: the
