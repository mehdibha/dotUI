  ...
  return async (tree) => {
    ...
    visit(tree, 'mdxJsxFlowElement', (node: MdxJsxFlowElementHast) => {
      if (node.name === 'Demo' || node.name === 'Example') {
        ...
        const engine =
          (extractStringAttribute(node, 'engine') as 'source' | null) ??
          'legacy'
    ...
    for (const interactiveDemo of successfulInteractiveDemos) {
      allImports.push({
        ...
        namedExport: interactiveDemo.isSourceDefault
          ? undefined
          : interactiveDemo.importName,
      })
    }
    ...
    if (isSource && rawSource) {
      const selections: ControlSelection[] = controls.map((c) => ({
        ...
        default:
          c.type === 'icon'
            ? null
            : (((c as { defaultValue?: unknown }).defaultValue as never) ??
              null),
      }))
      codeTemplate = await buildSourceOverlay({
        source: rawSource,
        controls: selections,
