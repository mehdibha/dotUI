 ...
 *  `encodeState`, bypassing its typing — for crafting hostile presets. */
...
describe("preset codec", () => {
  ...
  it("round-trips a modified state", () => {
    ...
    expect(decoded(decodeState(encoded ?? ""))).toEqual(state)
  })
  ...
  it("rejects garbage and other versions", () => {
    for (const encoded of [
      ...
      expect(decodeState(encoded).ok).toBe(false)
  })
  ...
  it("rejects a deflate bomb without inflating it", () => {
    ...
    expect(decodeState(bomb)).toEqual({
      ok: false,
  ...
  for (const preset of PRESETS) {
    it(`encode∘decode is byte-identity for the ${preset.name} preset`, () => {
      ...
      expect(encodeState(decoded(decodeState(encoded)))).toBe(encoded)
    })
  }
