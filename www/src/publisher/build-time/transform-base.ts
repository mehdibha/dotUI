...
function applyTransform(sourceFile: SourceFile, ctx: ApplyContext): void {
	...
	for (const call of sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
		...
		if (!callee.isKind(SyntaxKind.Identifier) || callee.getText() !== "useStyles") continue;
...
function findNextVariantProps(sourceFile: SourceFile, oldStylesTypeIdent: string) {
	...
	for (const ewta of sourceFile.getDescendantsOfKind(SyntaxKind.ExpressionWithTypeArguments)) {
...
function ensureTailwindVariantsImport(sourceFile: SourceFile): void {
	const existing = sourceFile.getImportDeclaration((imp) => imp.getModuleSpecifierValue() === "tailwind-variants");
	if (existing) {
...
function ensureNamedImport(imp: ImportDeclaration, name: string, isTypeOnly: boolean): void {
