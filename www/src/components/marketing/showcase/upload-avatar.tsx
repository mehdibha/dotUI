...
export function UploadAvatar({ className, ...props }: React.ComponentProps<"div">) {
	return (
			...
			<CardContent>
				<DropZone className="w-full gap-3 py-6">
					<Avatar size="lg" className="size-16">
						<AvatarImage src="https://github.com/mehdibha.png" alt="Profile photo" />
						<AvatarFallback>
							<ImageUpIcon className="size-5 text-fg-muted" />
						</AvatarFallback>
					</Avatar>
					<DropZoneLabel className="sr-only">Drag a file here to upload</DropZoneLabel>
					<FileTrigger acceptedFileTypes={["image/png", "image/jpeg"]}>
						<Button variant="default" size="sm">
							Choose file
						</Button>
					</FileTrigger>
					<p className="min-w-0 text-center text-xs text-fg-muted">PNG or JPG, up to 2MB</p>
				</DropZone>
			</CardContent>
