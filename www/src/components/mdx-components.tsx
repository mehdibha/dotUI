...
export const Link = ({ className, href, ref: _, children, ...props }: React.ComponentProps<"a">) => {
...
export const components: React.ComponentProps<typeof MDXContent>["components"] = {
  ...
  blockquote: ({ className, ...props }) => (
    <blockquote className={cn("[&>*]:text-fg-muted mt-6 border-l-2 pl-6 italic", className)} {...props} />
  ),
  ...
  Step: ({ className, ...props }: React.ComponentProps<"h3">) => (
    <h3
      className={cn("font-heading mt-8 scroll-m-20 text-base font-semibold tracking-tight", className)}
      {...props}
    />
  ),
  ...
  ColorBadge: ({ className, children, ...props }: React.ComponentProps<"div">) => (
    <div
      className="bg-bg-muted inline-flex items-center space-x-1.5 rounded border px-2 py-0.5 font-mono text-sm"
      {...props}>
