...
export function ThemesPreview() {
  const { currentThemeId, setCurrentThemeId } = useThemes();
  const [touched, setTouched] = React.useState(false);

  React.useEffect(() => {
    const interval = setInterval(() => {
      if (touched) return;
      const currentIndex = dotUIThemes.findIndex(
        (theme) => theme.id === currentThemeId
      );
      const nextIndex = (currentIndex + 1) % dotUIThemes.length;
      setCurrentThemeId(dotUIThemes[nextIndex].id);
    }, 5000);

    return () => clearInterval(interval);
  }, [currentThemeId, setCurrentThemeId, touched]);

  return (
    <div className="relative">
      <div className="absolute left-1/2 top-0 flex w-[100vw] -translate-x-1/2 -translate-y-[calc(100%+20px)] flex-col items-center justify-center">
        <div className="relative translate-x-40">
          ...
          <span className="text-fg-muted w-30 font-josefin absolute right-0 top-0 translate-x-[150px] text-sm font-bold">
            Give it a shot!
          </span>
        </div>
        <Tabs
          variant="solid"
          selectedKey={currentThemeId}
          onSelectionChange={(key) => {
            setCurrentThemeId(key as string);
            setTouched(true);
          }}
        >
          <TabList className="justify-center bg-transparent">
            {dotUIThemes.map((theme) => (
              <Tab
                key={theme.id}
                id={theme.id}
      ...
      />
      <div className="px-12">
        <ThemeOverride className="rounded-md border duration-300">
          <ComponentsOverview />
        </ThemeOverride>
      </div>
    </div>
