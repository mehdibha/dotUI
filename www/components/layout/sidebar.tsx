            ...
            >
              <StyledTooltip content="Login" isDisabled={!isCollapsed}>
                <SidebarButton href="/login" variant="primary" size="sm">
                  ...
                  <span className="text-center group-data-collapsed/sidebar:flex-1">
                    Login
                  </span>
                </SidebarButton>
              </StyledTooltip>
...
const SidebarRoot = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative z-10 h-svh w-(--sidebar-width) transition-sidebar group-data-collapsed/sidebar:w-(--sidebar-width-collapsed)",
        )}
      />
      <div className="fixed inset-y-0 left-0 z-10 flex h-svh w-(--sidebar-width)  flex-col overflow-hidden border-r transition-sidebar group-data-collapsed/sidebar:w-(--sidebar-width-collapsed) [&_svg]:text-fg-muted">
        <div className="relative flex h-svh w-(--sidebar-width) flex-1 translate-x-[-0.5px] bg-card/50 flex-col overflow-hidden">
          {children}
        </div>
      </div>
    </aside>
...
function PageNode({
  ...
  return (
    <Link
      ...
      href={url as Route}
      className={cn(
        "border-bg-muted group block border-l py-1 pl-4 font-medium text-fg-muted transition-colors hover:text-fg",
        {
