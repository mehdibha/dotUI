...
type HeaderItem = "logo" | "menu" | "mobileNav" | "search" | "github" | "themeToggle";
...
export function Header({
	...
	return (
		<header className={cn("sticky top-0 z-30 h-(--header-height) w-full border-b bg-bg", className)}>
			<div className={cn("container relative flex h-full items-center justify-between", containerClassName)}>
				<div className="flex items-center gap-3 md:gap-6">
					...
					{isVisible("menu") && (
						<nav className="flex items-center gap-3 text-sm max-md:hidden">
							{navItems.map((item) => (
								<Link key={item.url} href={item.url} className="px-0.5 text-fg-muted transition-colors hover:text-fg">
									{item.name}
								</Link>
							))}
						</nav>
